...
| 3 | **Extend QA evaluation** — priority corrected (Step 155): **CoQA > SQuAD v2 > TruthfulQA** — these have published SE/SC baselines to compare against; AmbigQA/PopQA have none. Folded into the replication-grid plan (loaders + presets) | Planned (Step 155 replication grid) |
...
| NGC image | `nvcr.io/nvidia/pytorch:25.01-py3` — **Pyxis only** (rootless Docker dead on power-gpu since 2026-07-01, cgroup v2 BPF block); named container `ngc_pytorch_2501` cached per-node (~8 min first time, instant after) |
