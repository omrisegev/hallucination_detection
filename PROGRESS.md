...
**Date**: 2026-07-05
**Last updated**: Step 153 (branch `experiment/item6-temperature`) — **Item 6 temperature variation COMPLETE (ran on Colab A100).** 9 runs × 200 MATH-500 / Qwen2.5-Math-7B. **Both pre-registered gates FAIL — that IS the finding, and it's clean.** Q2 (primary): paired **AUC(B multi-T) − AUC(A same-T=1.0) = −0.053 [−0.103, −0.011]** — temperature diversity does *not* help, it hurts; while **AUC(A) − AUC(single pass) = +0.061 [+0.004, +0.128]** — more same-T passes *do* help. So the multi-pass lift is **variance reduction from repeated sampling at T≈1.0, not temperature diversity** (A off-diag ρ +0.45 = same signal + independent noise; B off-diag ρ +0.01 = the off-temp passes are near-random, not diverse). Q1 (single-pass AUROC vs T): inverted-U — 0.545 / 0.644 / 0.851 / 0.878 / 0.629 at T=0.3/0.6/1.0/1.5/2.0 — but confounded by accuracy collapsing 80%→4%, so G-T1 FAILs (T=1.5 higher point-estimate 0.878 but overlapping CIs, 27.5% acc). **Two methodological flags** for follow-up: `spectral_entropy` sign flips at hot T (0.261@T=1.0 with the fixed −1 sign), and the label-free L-SML fusion underperforms the best single feature at every T (fused 0.851 vs `cusum_max` 0.927 @ T=1.0; the `epr` anchor is weak at low T → fragile sign orientation). **Data-debt repaid**: T=1.0 run0 is now the canonical MATH-500/Qwen-7B raw-trace cache (entropies + spilled energies + top-50 logprobs + token ids, N=200), unblocking the Extension E streaming replication. Results embedded in the committed notebook + `cache/phase15_temperature/results/phase15_results.pkl` (keeps every raw score → all follow-ups are CPU-only). Next: **download the 9 caches and run the CPU follow-ups** (see Research_Directions Item 6 — self-consistency baseline is the priority). Note: the results notebook had been saved onto the main tree (`experiment/bocpd-features`) by accident and was moved back onto this branch.
...
| 6 | **Experiment 2 — Temperature variation** — run same model at T∈{0.3,0.6,1.0,1.5,2.0}; does higher T improve detectability? Ablate: T-diversity vs just more passes | ✅ **Complete (Step 153)** — ran on A100. Both gates FAIL: multi-pass lift is variance reduction at T≈1.0, **not** T-diversity (paired B−A = −0.053 [−0.103,−0.011]; A−single = +0.061 [+0.004,+0.128]). CPU follow-ups queued (see Item 6) |
...
### Priority 2 — Temperature variation experiment (Item 6) — ✅ RAN (Step 153); CPU follow-ups queued

**`notebooks/Spectral_Analysis_Phase15_Temperature.ipynb` on branch `experiment/item6-temperature`, ran on A100 (Step 153).** 9 runs × 200 MATH-500 / Qwen2.5-Math-7B. **Both gates FAIL — clean negative result.** Q2 primary: paired **AUC(B multi-T) − AUC(A same-T) = −0.053 [−0.103,−0.011]** (diversity hurts) vs **AUC(A) − AUC(single) = +0.061 [+0.004,+0.128]** (more same-T passes help) → the lift is variance reduction at T≈1.0, not temperature diversity. Q1 AUROC-vs-T inverted-U (0.545/0.644/0.851/0.878/0.629) confounded by accuracy collapse 80%→4%. Surfaced two method flags: `spectral_entropy` sign flips at hot T; L-SML fusion < best single feature at every T (weak `epr` anchor at low T). T=1.0 run0 is now the canonical raw-trace cache (Extension E debt repaid). Results in the committed notebook + `phase15_results.pkl`.

**CPU follow-ups (download the 9 caches, then all local — see Research_Directions Item 6 for the full menu, priority order):**
1. **Self-consistency / semantic-entropy baseline** over the 5 T=1.0 passes — the reviewer-mandatory "does spectral beat sample-5×-and-vote?" test; also closes Item 5. **Do this first.**
2. **K-sweep** AUROC(A) for K=1..5 — cost/benefit curve for "how many passes".
3. **Anchor/sign robustness across T** — re-fuse with `cusum_max` anchor / per-feature label-free sign / drop `spectral_entropy`; likely recovers the low-T gap and closes fusion-vs-best-single.
4. **New feature families from saved data** — ΔE spilled-energy spectral features + top-50 logprob features (margin, varentropy, Rényi).
5. **Fairer diversity set** (drop degenerate T=2.0) + **streaming earliest-prefix replication** (Extension E, now unblocked by the fresh raw cache).
...
| `experiment/item6-temperature` | **Active (this branch)** — worktree `../hallucination_detection-item6`; pushed at HEAD | Steps 152–153: Phase 15 temperature notebook + **results** + generate_full top-k logprobs + paired_boot_delta_auc + multipass_lsml_continuous. Phase 15 Cell 1 clones THIS branch. Ready for careful merge to master once the CPU follow-ups are done |
| `experiment/pivot-alternatives` / `experiment/bocpd-features` | Active (main working tree, parallel session) | Step 151+: anomaly_utils + temporal_models + BOCPD pivot pilot helpers. (The Phase 15 results notebook briefly landed here by mistake — moved back to item6 in Step 153) |
