...
def code_freeze(d):
    paths=[Path(__file__).with_name(n) for n in ['core.py','em.py','data.py','runner.py']]
...
def predictions(d,ps,test,task,models,matrices,inner=False):
    ...
    for (enc,kind),model in models.items():
        ...
        for i in test:
            if task=='pb':
                score,fail=location(model,ps[i],enc);modes.append(int(score.argmax()));medians.append(int(np.searchsorted(np.cumsum(score),.5)))
            else:
