...
# Readout-family extension (Step 429): the same three soft models fitted on the per-step
# softmax MASS (the jumps of the cumulative curve) instead of the cumulative curve itself.
# The review of cumulative-vote v2 (section 3) traced the loss of the learned soft weights to
# the shared monotone ramp of the cumulative encoding; fitting on the pmf removes that ramp.
PMF_ARMS = [('pmf','equal'), ('pmf','spectral'), ('pmf','continuous_lsml')]
...
def encode(profile, encoding, task):
    ...
    if task=='pb':
        ...
        mass=softmax(z,axis=0)
        if encoding=='pmf': return mass
        return 2*np.cumsum(mass,axis=0)[:-1]-1
...
def location(model,profile,encoding):
    ...
    if encoding=='pmf':
        # Fused per-step score of the mass rows; the location mass is the score lifted to be
        # non-negative and normalised (no PAVA: the pmf is not a cumulative curve).
        x=encode(profile,'pmf','pb');scores=model.predict(x)
        fail=model.status!='ok' or not np.isfinite(scores).all()
        if fail:scores=x.mean(1)
        mass=scores-scores.min()
        if not np.isfinite(mass).all() or mass.sum()<=1e-12:mass=np.ones(S);fail=True
