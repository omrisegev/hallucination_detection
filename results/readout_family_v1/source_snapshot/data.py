...
from .core import CHANNELS, READOUTS, ARMS, PMF_ARMS, profiles
...
class Dataset:
    def __init__(self,c):
        ...
        self.profiles=None
        # Readout-family extension (Step 429).  Absent keys reproduce the frozen v2 behaviour.
        self.readouts=list(READOUTS)+list(c.get('readouts_ext',[]))
        self.extended=bool(c.get('readouts_ext'))
        self.rosters=[tuple(r) for r in c.get('rosters',[['top5','all'],['selected','all'],['shuffle','all']])]
        self.pb_extra_rosters=[tuple(r) for r in c.get('pb_extra_rosters',[['top5','errors']])]
        self.fusion_arms=list(ARMS)+(list(PMF_ARMS) if c.get('pmf_arms') else [])
        self.profile_file='profiles_full.npy' if self.extended else 'profiles.npy'
        self.shuffle_file='shuffled_full.npy' if self.extended else 'shuffled_top5.npy'
...
def prepare(d):
    ...
    if not (d.out/'PROFILES_COMPLETE.json').exists():_build_legacy(d)
    done=json.loads((d.out/'PROFILES_COMPLETE.json').read_text(encoding='utf8'))
    for key,recorded in [('frozen_profiles_sha256',done['profiles_sha256']),('frozen_shuffle_sha256',done['shuffle_sha256'])]:
        if key in d.c and d.c[key]!=recorded:raise ValueError(f'{key}: rebuilt frozen profiles differ from the reference run ({recorded})')
    if d.extended and not (d.out/'PROFILES_EXT_COMPLETE.json').exists():_build_extended(d)

def _build_extended(d):
    """profiles_full.npy = the seven frozen readouts (copied bit-for-bit) + the extended ones;
    shuffled_full.npy uses the SAME per-answer permutations as shuffled_top5.npy."""
    sys.path.insert(0,str(Path(__file__).resolve().parents[3]))
    from spectral_utils.step_readouts_v1 import EXT_READOUTS,ext_profiles,robust_standardize_tokens
    if list(d.c['readouts_ext'])!=EXT_READOUTS:raise ValueError('readouts_ext must equal step_readouts_v1.EXT_READOUTS')
    paths=d.c['paths'];z=np.load(paths['tokens']);tokens=z['tokens'];toff=z['token_offsets'];spans=z['step_spans']
    cfg={k:d.c[k] for k in ['frac_z_threshold','boxcar_width','page_k','quantile'] if k in d.c}
    legacy=np.load(d.out/'profiles.npy',mmap_mode='r');legacy_shuffle=np.load(d.out/'shuffled_top5.npy',mmap_mode='r')
    K=len(d.readouts);shape=(int(d.off[-1]),11,K);L=len(READOUTS)
    full=np.lib.format.open_memmap(d.out/'profiles_full.npy',mode='w+',dtype='float64',shape=shape)
    shuffled=np.lib.format.open_memmap(d.out/'shuffled_full.npy',mode='w+',dtype='float64',shape=shape)
    started=time.perf_counter()
    for i in range(d.n):
        a,b=d.off[i:i+2];ta,tb=toff[i:i+2];x=tokens[ta:tb].astype(float);sp=spans[a:b]
        full[a:b,:,:L]=legacy[a:b];full[a:b,:,L:]=ext_profiles(robust_standardize_tokens(x),sp,cfg)
        rng=np.random.default_rng(np.random.SeedSequence([d.c['seed'],i]))
        perm=np.column_stack([rng.permutation(x[:,j]) for j in range(11)])
        shuffled[a:b,:,:L]=profiles(perm,sp);shuffled[a:b,:,L:]=ext_profiles(robust_standardize_tokens(perm),sp,cfg)
        if not np.array_equal(shuffled[a:b,:,0],legacy_shuffle[a:b]):raise AssertionError(f'shuffle permutation drifted at answer {i}')
        if i%1000==0:print(f'extended profiles {i}/{d.n}, {time.perf_counter()-started:.1f}s',flush=True)
    full.flush();shuffled.flush()
    dump(d.out/'PROFILES_EXT_COMPLETE.json',{'seconds':time.perf_counter()-started,'readouts':d.readouts,'reducer_config':cfg,
         'profiles_full_sha256':digest(d.out/'profiles_full.npy'),'shuffled_full_sha256':digest(d.out/'shuffled_full.npy'),
         'legacy_profiles_sha256':digest(d.out/'profiles.npy'),'legacy_shuffle_sha256':digest(d.out/'shuffled_top5.npy'),
         'note':'columns :7 are the frozen readouts copied from profiles.npy; shuffle uses the identical SeedSequence([seed,i]) permutations'})
