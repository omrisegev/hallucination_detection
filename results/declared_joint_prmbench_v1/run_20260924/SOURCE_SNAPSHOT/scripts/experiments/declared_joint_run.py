"""declared_joint_prmbench_v1: DECLARED provenance partitions (family-equal and Joint L-SML) on the
11-channel step bank and its four-channel extension, PRMBench first.
Frozen protocol: results/declared_joint_prmbench_v1/PROTOCOL.json.
...
INPUTS = {'level_bank': TPF / 'DERIVATIVE_CHANNELS.npz', 'ct7_tokens': MAIN / 'results/ct7_token_lsml_v1/CT7_TOKEN_MATRICES.npz',
          ...
          'baseline_scores': DEPTH / 'results/step_level_bank_baseline_v1/STEP_SCORES.npz', 'pool_z': SCR / 'pool_z.npy', 'pool_names': SCR / 'pool_names.json', 'pool_structure': ROOT / 'results/indbank_lsml_prmbench_v1/POOL_STRUCTURE.csv', 'hist29_profiles': SCR / 'hist29_top10_profiles.npy', 'folds_v2': MAIN / 'results/localization_source_group_audit_v1/FOLDS_V2.json'}
...
# ------------------------------------------------------------------ channels: bank11 + the four named channels
t = time.perf_counter()
POOL = np.load(INPUTS['pool_z']); PN = json.loads(INPUTS['pool_names'].read_text(encoding='utf8')); assert POOL.shape == (int(off[-1]), len(PN))
names11 = PN[:11]; assert names11 == list(np.load(INPUTS['level_bank'])['channels'].astype(str))
ADD = P['channels_added']['list']
names = names11 + ADD
raw = POOL[:, [PN.index(c) for c in names]]; assert np.isfinite(raw).all()
missing = {}
values = answer_standardize(raw, off)
BANKS = {'B11': list(range(11)), 'B15': list(range(15))}
DECL = {}
for bank in BANKS:
    fam = P['declared_partitions'][bank + (' (5/3/3)' if bank == 'B11' else ' (5/4/3/3)')]
    lab = np.full(len(BANKS[bank]), -1, int)
    for gi, (_, members_) in enumerate(fam.items()):
        for c in members_: lab[names.index(c)] = gi
    assert (lab >= 0).all() and len(np.unique(lab)) >= 3 and np.bincount(lab).min() >= 3, (bank, lab)
    DECL[bank] = lab
timing['channels_s'] = time.perf_counter() - t
dump(OUT / 'INPUT_MANIFEST.json', {k: {'path': str(v), 'bytes': v.stat().st_size, 'sha256': sha(v)} for k, v in INPUTS.items()} | {'channels': names, 'added': ADD, 'declared_partitions': {b: DECL[b].tolist() for b in BANKS}, 'missing_step_values_filled': missing, 'population': {'answers': n, 'prm': int(prm.sum()), 'eligible': int(eligible.sum()), 'noncontrol': int(noncontrol.sum()), 'steps': int(off[-1])}})
snap = OUT / 'SOURCE_SNAPSHOT'; code = {}
for rel, base in [('scripts/experiments/declared_joint_run.py', ROOT), ('spectral_utils/lsml_gate_locator_research.py', DEPTH), ('spectral_utils/fusion_utils.py', DEPTH)]:
...
print('channels ready in {:.0f}s; banks {}; declared sizes {}'.format(timing['channels_s'], [(b, len(c)) for b, c in BANKS.items()], [(b, np.bincount(DECL[b]).tolist()) for b in BANKS]), flush=True)
...
for k in FOLDS:
    ...
    for bank, cols in BANKS.items():
        ...
        dl = DECL[bank]; dK = len(np.unique(dl)); dsz = np.bincount(dl); wd = 1.0 / (dK * dsz[dl])
        arms = [('lsml', w), ('equal', we), ('discovered_group_equal', wg), ('declared_equal', wd)]
        try:
            wj, mj = fit_fusion_weights(X[fit_rows], FusionRecipe(name=bank + '_joint', members=members, mode='joint', groups=tuple(dl.tolist()), anchor=0), seed=FIT_SEED)
            if not mj['converged']:
                failures.append({'fold': k, 'bank': bank, 'arm': 'declared_joint', 'reason': 'joint not converged, starts=' + str(mj['converged_starts']) + ' misfit=' + str(mj['relative_offdiag_misfit'])})
            arms.append(('declared_joint', wj))
            fit_log.append({'fold': k, 'cal_fold': cal, 'fit_folds': fitf, 'bank': bank + '_joint', 'K': dK, 'groups': dl.tolist(), 'group_sizes': dsz.tolist(), 'weights': np.round(wj, 5).tolist(),
                            'weight_ipr': float(1 / np.sum((np.abs(wj) / np.abs(wj).sum()) ** 2)), 'anchor_spearman': mj['anchor_spearman'], 'anchor_flipped': mj['anchor_flipped'],
                            'converged': bool(mj['converged']), 'converged_starts': int(mj['converged_starts']), 'multistart': mj['multistart'],
                            'relative_offdiag_misfit': float(mj['relative_offdiag_misfit']),
                            'cross_group_weights': [float(v) for v in mj['joint_weight_meta']['cross_group_weights']],
                            'jacobian': mj['jacobian'], 'fit_steps': int(len(fit_rows)), 'members': list(members)})
        except Exception as e:
...
for m in METHODS:
    ...
    conf[m] = c
    el = np.isfinite(auc[m]); ps = float(.5 * (official[m]['f1'] + official[m]['negative_f1'])) if all(np.isfinite(list(thr.values()))) else np.nan
...
diag['replay'] = rep; diag['calibration_thresholds'] = cal_thr; diag['declared_partitions'] = {b: DECL[b].tolist() for b in BANKS}; diag['added'] = ADD
diag['joint'] = {b: [{'fold': f['fold'], 'converged': f['converged'], 'starts': f['converged_starts'], 'misfit': f['relative_offdiag_misfit'], 'cross_group_weights': f['cross_group_weights']} for f in sorted([f for f in fit_log if f['bank'] == b + '_joint'], key=lambda f: f['fold'])] for b in BANKS}
diag['added_block_weight_B15'] = {a: [float(np.sum(np.abs(f['weights'][11:]))) for f in sorted([f for f in fit_log if f['bank'] == ('B15_joint' if a == 'declared_joint' else 'B15')], key=lambda f: f['fold'])] for a in ['lsml', 'declared_joint']}
...
prim = [('B11_declared_equal', 'B11_equal'), ('B11_declared_joint', 'B11_declared_equal'), ('B15_declared_equal', 'B11_declared_equal')]
sec = [(b + '_' + a, b + '_equal') for b in ['B11', 'B15'] for a in ['lsml', 'discovered_group_equal', 'declared_equal', 'declared_joint']]
sec += [(b + '_declared_equal', b + '_discovered_group_equal') for b in ['B11', 'B15']] + [(b + '_declared_equal', b + '_lsml') for b in ['B11', 'B15']]
sec += [(b + '_declared_joint', b + '_lsml') for b in ['B11', 'B15']] + [('B15_' + a, 'B11_' + a) for a in ['equal', 'lsml', 'declared_equal', 'declared_joint']]
sec += [(b + '_' + a, 'ct7') for b in ['B11', 'B15'] for a in ['lsml', 'declared_equal', 'declared_joint']] + [('B11_lsml_replay4', 'ct7')]
K = 6; rows = []; deltas = {}
for a, b in prim + sec:
