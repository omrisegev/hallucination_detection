...
OUT = ROOT / "results/prmbench_lsml_month_audit_v3"
...
def main():
    ...
    # Retrospective family:5 contrasts x3 endpoints (within/raw PRM/z PRM).
    family = 15
    for a,b in pairs:
        ...
        for endpoint,d in draws.items():
            ...
            comparisons.append({"a":a,"b":b,"endpoint":endpoint,"delta":float(point),
                                ...
                                "ci_bonferroni15":np.quantile(d,[.025/family,1-.025/family]).tolist(),
    ...
    sources = paths+[jp,npz,folds_path,meta_path,module_path,fit_path,helper_path,bank_path,evidence_path,evidence_answers,Path(__file__)]
    ...
    print(json.dumps({"rows":rows,"seconds":report["seconds"]},indent=2),flush=True)
