...
for i in range(n):                                                     # step_spans are ANSWER-relative: slice the answer's tokens first
    ta, tb = toff[i:i+2]; xb = XB[ta:tb].copy(); xb[~VB[ta:tb]] = np.nan; rk = risk_tok[ta:tb]
    for s in range(off[i], off[i+1]):
        a, b = spans[s]
        for j in range(4): add[s, j] = top10(xb[a:b, j])
        add[s, 4] = top10(rk[a:b])
assert spans[off[:-1], 0].max() == 0 and np.all(spans[off[1:] - 1, 1] == np.diff(toff))
