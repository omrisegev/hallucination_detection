"""Minimax initial-factor retention ranking with checked regrouping per deletion."""
import numpy as np
...
def refine_minimax(values,labels,row_folds,*,seed,group_seed,notify=None):
    ...
    for step in range(limit+1):
        ...
        if step==limit:break
        info,loss=information(subcov,ref_a[active])
        # Rank cumulative worst-factor loss against the same fixed reference
        # used by the admissibility constraint. No current-factor reset.
        importance=np.max(1-(info[relevant]-loss[:,relevant])/reference[relevant],axis=1)
        ...
        for index in candidates[:3]:
            ...
            audit.append(dict(removed=int(active[index]),worst_initial_information_loss=float(importance[index]),
    ...
    selection=dict(path=path,automatic_index=automatic_index,automatic=chosen,deletion_audit=audit,
        ...
        stopping_rule='last accepted information-feasible state',ranking='minimize worst initial-factor cumulative information loss',group_seed=group_seed,fit_seed=seed,reference='fixed initial factors; never reset after regrouping')
