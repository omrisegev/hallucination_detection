...
"""
import numpy as np
from .joint_mass_polish import polish_kernel_mass
...
def feature_mass(covariance):
    ...
    initial_raw=raw.copy();initial_kkt=kkt;polishing=None
    if np.isfinite(raw).all() and raw.sum()>0 and kkt>1e-6:
        raw,polishing=polish_kernel_mass(kernel,raw,tolerance=1e-8,max_iterations=1000)
    ...
    return raw/raw.sum(),dict(raw_mass=raw,mass=raw/raw.sum(),kernel=kernel,
        initial_raw_mass=initial_raw,initial_projected_gradient=initial_kkt,polishing=polishing,
