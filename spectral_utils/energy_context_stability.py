...
def simplex_qp(Q,r):
    """Same active-face objective as S0, without an absolute objective deadband.

    A gain smaller than 1e-13 can correspond to a KKT violation above 1e-7.
    Keep the original feasibility/KKT tolerances; do not fall back to equal.
    """
    Q=np.asarray(Q,float);r=np.asarray(r,float)
    if Q.ndim==2:Q=Q[None]
    if r.ndim==1:r=np.broadcast_to(r,(len(Q),len(r)))
    n,m,_=Q.shape;best=np.full(n,np.inf);result=np.full((n,m),np.nan)
    for size in range(1,m+1):
        for face in combinations(range(m),size):
            ids=np.array(face);system=np.zeros((n,size+1,size+1))
            system[:,:size,:size]=Q[:,ids[:,None],ids]
            system[:,:size,size]=1.;system[:,size,:size]=1.
            rhs=np.column_stack([r[:,ids],np.ones(n)])
            z=np.linalg.solve(system,rhs[...,None])[... ,0][:,:size]
            valid=np.all(z>=-1e-10,axis=1)
            w=np.zeros((n,m));w[:,ids]=z
            value=.5*np.einsum('ni,nij,nj->n',w,Q,w)-np.sum(w*r,axis=1)
            update=valid & (value<best)
    ...
    grad=np.einsum('nij,nj->ni',Q,result)-r;level=np.sum(result*grad,axis=1)
    violation=np.max(np.where(result>1e-8,np.abs(grad-level[:,None]),np.maximum(level[:,None]-grad,0.)))
