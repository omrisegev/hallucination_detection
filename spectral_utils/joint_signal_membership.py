"""Step407 global-signal membership variant of frozen Steps404-405 selection.
...
def learn_signal_structure(train,rowfold,grams,nrows,*,seed,calibration_seed,notify=None):
    ...
    for round_index in range(3):
        ...
        local,signal_audit=signal_group_support(local,np.asarray(discovery['labels']),audit)
...
def fit_signal_aware_joint(values,offsets,source_ids,inner_folds,*,anchor_index,seed=399170,notify=None):
    ...
    try:
        ...
        membership=learn_signal_structure(z,np.repeat(inner_folds,np.diff(offsets)),gram,int(sizes.sum()),
