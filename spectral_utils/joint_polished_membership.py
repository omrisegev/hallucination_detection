"""Step412 staged membership with group rediscovery during refinement."""
import numpy as np
from .joint_sparse_membership import exact_aliases,alias_coordinates,source_grams
from .joint_staged_membership import score_noise_aware_joint
from .joint_polished_structure import learn_polished_structure
from .joint_polished_refinement import refine_polished
from .lsml_gate_locator_research import _orient


def fit_polished_joint(values,offsets,source_ids,inner_folds,*,anchor_index,seed=399170,notify=None):
    ...
    try:
        ...
        membership=learn_polished_structure(z,np.repeat(inner_folds,np.diff(offsets)),gram,int(sizes.sum()),
        ...
        fit=refine_polished(z[:,initial],part,np.repeat(inner_folds,np.diff(offsets)),
