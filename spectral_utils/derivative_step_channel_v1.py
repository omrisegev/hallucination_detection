...
def ema(values: np.ndarray, window: int = EMA_WINDOW) -> np.ndarray:
    ...
    if x.ndim != 1:
        raise ValueError("ema expects a 1-d token series")
...
def derivative_step_readout(matrix: np.ndarray, spans: np.ndarray,
    ...
    for c in range(n_channels):
        series = x[:, c]
        if not np.isfinite(series).all() or series.std() <= 1e-12:
            continue
        rises = np.diff(ema(series, window), prepend=series[0])
        ...
        for s, (a, b) in enumerate(spans):
            ...
            out[s, c] = np.partition(seg, len(seg) - k)[-k:].mean()
    return out
