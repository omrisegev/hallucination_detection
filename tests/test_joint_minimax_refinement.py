...
from spectral_utils.joint_minimax_refinement import refine_minimax
...
def test_budget_rejection_precedes_group_discovery():
    ...
    with patch('spectral_utils.joint_minimax_refinement.discover_sourcefold_groups',
               side_effect=AssertionError('Budget violation must not reach discovery')) as discover:
        fit=refine_minimax(x,labels,folds,seed=410370,group_seed=410270)
...
def test_accepted_path_always_preserves_fixed_initial_information():
    ...
    with patch('spectral_utils.joint_minimax_refinement.discover_sourcefold_groups',side_effect=partition):
        fit=refine_minimax(x,labels,folds,seed=410370,group_seed=410270)
    ...
    for step,state in enumerate(s['path']):
        ...
        if step+1<len(s['path']):
            # Independent reduced-system solves verify the preferred deletion
            # at every changing active set, not the production loss identity.
            losses={}
            for index,feature in enumerate(ids):
                if np.sum(labels[ids]==labels[feature])<=2:continue
                proposal=np.delete(ids,index)
                after=np.sum(a[proposal]*np.linalg.solve(reg[np.ix_(proposal,proposal)],a[proposal]),axis=0)
                losses[int(feature)]=float(np.max(1-after/initial))
            chosen=s['deletion_audit'][step]
            assert not chosen['rejected_candidates']
            assert losses[chosen['removed']]<=min(losses.values())+1e-7
            np.testing.assert_allclose(chosen['worst_initial_information_loss'],losses[chosen['removed']],atol=1e-6)
