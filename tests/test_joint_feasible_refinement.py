...
from spectral_utils.joint_feasible_refinement import refine_feasible
...
def test_budget_rejection_precedes_group_discovery():
    ...
    with patch('spectral_utils.joint_feasible_refinement.discover_sourcefold_groups',
               side_effect=AssertionError('Budget violation must not reach discovery')) as discover:
        fit=refine_feasible(x,labels,folds,seed=410370,group_seed=410270)
...
def test_accepted_path_always_preserves_fixed_initial_information():
    ...
    with patch('spectral_utils.joint_feasible_refinement.discover_sourcefold_groups',side_effect=partition):
        fit=refine_feasible(x,labels,folds,seed=410370,group_seed=410270)
