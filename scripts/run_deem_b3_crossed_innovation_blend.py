#!/usr/bin/env python3
"""Run target-free seed-0 conservative crossed-innovation blend."""
...
from spectral_utils.deem_b3_crossed_innovation_blend import ARMS,BLEND,fit_blended_innovation
...
SCHEMA="deem_b3_crossed_innovation_blend_seed0_2026_08_25"
def fit_cell(payload):
 ...
 for arm in ARMS:
  result,innovation=fit_blended_innovation(prepared,arm,seed=0)
  ...
  meta={"schema":SCHEMA+"_fit","cell_id":cell,"dataset_family":bundle.dataset_family,"arm":arm,"npz_sha256":digest,"health":result.health,"source_bundle_sha256":bundle.bundle_sha256,"run_definition_sha256":definition_hash,"labels_accessed_during_fit":False,"target_module_imported_during_fit":False}; meta["content_sha256"]=canonical_sha256(meta); meta_path=path.with_suffix(".json"); atomic_write_json(meta_path,meta)
...
def main():
 ...
 records.sort(key=lambda row:(row["cell_id"],row["arm"])); assert len(records)==48; freeze={"schema":SCHEMA+"_freeze","run_definition_sha256":definition["content_sha256"],"records":records,"all_healthy":True,"labels_accessed_during_fit":False,"target_module_imported_during_fit":False}; freeze["content_sha256"]=canonical_sha256(freeze); atomic_write_json(args.out_dir/"SCORE_FREEZE.json",freeze); print("frozen",len(records),"fits")
