"""Mechanism checks for Renyi-view fusion v2; not benchmark performance tests.
...
from spectral_utils.renyi_view_fusion_v2 import (ALPHAS, COLUMN_NAMES, K, METHODS, NOT_APPLICABLE, VIEW_NAMES,
...
class FitTests(unittest.TestCase):
    def test_alpha_grid_and_groups(self):
        self.assertEqual(ALPHAS, (0.1, 0.25, 0.5, 1.0, 2.0, np.inf)); self.assertNotIn(4.0, ALPHAS)
        from spectral_utils.renyi_view_fusion_v2 import GROUP_OF, seed_for
        self.assertEqual([GROUP_OF[c] for c in COLUMN_NAMES], [0, 0, 0, 1, 1, 1, 2, 2, 2])
        self.assertEqual(seed_for('a'), seed_for('a')); self.assertNotEqual(seed_for('a'), seed_for('b'))

    def test_joint_seed_determinism(self):
        f1, _, _ = fit_all(self.lp, self.chosen, methods=('R6_sel__joint',), uid='u1')
        f2, _, _ = fit_all(self.lp, self.chosen, methods=('R6_sel__joint',), uid='u1')
        np.testing.assert_array_equal(f1['R6_sel__joint']['score'], f2['R6_sel__joint']['score'])
    ...
    def test_all_fits_reconstruct(self):
        ...
        X, names = bank_matrix(self.lp, self.chosen, 'R6_sel')
        ...
        np.testing.assert_array_equal(fits['view__H1']['score'], X[:, 3])
        for name in ('R6__equal', 'R6__iu'):
            self.assertTrue(np.all(fits[name]['weights'][6:] == 0))
        j = fits['R6_sel__joint']['diagnostics']
        self.assertEqual(j['group_sizes'], {'tail_orders_lt1': 3, 'head_orders_ge1': 3, 'selected_token': 3})
        for key in ('converged', 'multistart_status', 'joint_relative_offdiag_misfit', 'hard_relative_offdiag_misfit',
                    'model_covariance_condition', 'map_ridge', 'jacobian_full_global_rank', 'seed'):
            self.assertIn(key, j)

    def test_iu_transposition(self):
        """upcr_fit expects features x samples: passing Z (not Z.T) must differ."""
        X, _ = bank_matrix(self.lp, self.chosen, 'R6_sel')
        ...
        fits, _, _ = fit_all(self.lp, self.chosen, methods=('R6_sel__iu',))
        w = fits['R6_sel__iu']['state']['w']
    ...
    def test_failure_paths(self):
        ...
        for m in ('R6__equal', 'R6__iu', 'R6_sel__equal', 'R6_sel__iu', 'R6_sel__shrink', 'R6_sel__joint'):
        ...
        self.assertEqual(set(failures), set(METHODS))
        # SEL constant but Renyi varying: R6 arms succeed, shrink and joint fail explicitly (SEL group gone).
        fits, failures, _ = fit_all(self.lp, np.full(len(self.lp), 2.0))
        self.assertIn('R6__iu', fits); self.assertIn('R6_sel__shrink', failures); self.assertIn('view__sel1', failures)
        self.assertIn('R6_sel__iu', fits); self.assertIn('R6_sel__joint', failures)
        self.assertIn('joint needs K>=3 groups', failures['R6_sel__joint'])

    def test_joint_not_applicable(self):
        self.assertIn('R6_sel__joint', METHODS); self.assertIn('R6__joint', NOT_APPLICABLE); self.assertIn('R6__shrink', NOT_APPLICABLE)
    ...
    def test_equal_is_oriented_mean(self):
        fits, _, _ = fit_all(self.lp, self.chosen, methods=('R6__equal',))
        f = fits['R6__equal']; X, _ = bank_matrix(self.lp, self.chosen, 'R6')
        Z, keep, _, _ = zscore_columns(X)
    ...
    def test_real_rows_fit(self):
        for key, lp, chosen in load_real_rows(limit=5):
            ...
            X, _ = bank_matrix(lp, chosen, 'R6_sel')
            for name, fit in fits.items():
                np.testing.assert_allclose(X @ fit['effective'] + fit['intercept'], fit['score'], atol=1e-8, rtol=1e-8)
