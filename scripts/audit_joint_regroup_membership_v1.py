...
from scripts.run_joint_regroup_membership_v1 import OUT,inputs,KINDS,training_inputs,bank,references,PAIRS
...
def audit_regroup_path(model,x,offsets,folds):
    ...
    c=np.cov(np.ascontiguousarray(values).T);discoveries=0;changed=0;rejections=0
    ...
    return dict(status='PASS',discoveries=discoveries,changed_partitions=changed,rejections=rejections)


def main():
    for p,h in json.loads((OUT/'MANIFEST.json').read_text())['hashes'].items():assert sha(p)==h,p
    test=runpy.run_path(str(ROOT/'tests/test_joint_regroup_refinement.py'));passed=[]
    ...
    for kind in KINDS:
        ...
        for outer in range(5):
            ...
            np.testing.assert_allclose(expected,scores[kind+'__regroup'][~rows],atol=1e-12,rtol=1e-12)
            max_score=max(max_score,float(np.max(np.abs(expected-scores[kind+'__regroup'][~rows]))))
            print('MODEL_PASS',kind,outer,flush=True)
        new=scores[kind+'__regroup'];old=scores['base__regroup']
    ...
    for k,passed_margin in result['practical_preservation'].items():
        c=result['primary_contrasts'][k+'__regroup minus base__regroup']
        assert passed_margin==(native[k]==13769 and c['pb']['low']>-.01 and c['within']['low']>-.002)
    error=float(np.max(np.abs(scores['base__regroup']-scores['base__old'])))
    assert error==result['baseline_replay_max_error']
    ci=result['primary_contrasts']['base__regroup minus base__previous']
