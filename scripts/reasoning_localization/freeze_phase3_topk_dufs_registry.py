#!/usr/bin/env python3
"""Freeze the P3K top-k DUFS control before score fitting."""
...
def main() -> None:
    ...
    names = families = None; cells = []
    for cell_id in p2r.PB_CELLS:
        source = by_cell[cell_id]; path = root / source["artifact_path"]
    ...
    registry = {
        "schema": "reasoning-localization-p3k-execution-v1", "status": "FROZEN_BEFORE_RUN",
        "experiment_id": run.EXPERIMENT, "variant_order": list(run.VARIANTS),
        "release_root": str(release), "cells": cells, "member_names": list(names),
        "member_families": list(families),
        "family_counts": {family: list(families).count(family) for family in sorted(set(families))},
        "fit_contract": "five grouped donor folds; top-k-only DUFS graph and LIU; held responses projection-only",
        "dufs_seeds": list(run.p3f.DUFS_SEEDS), "dufs_epochs": run.p3f.DUFS_EPOCHS,
        "graph_k": run.p3f.GRAPH_K, "liu_lambda": run.p3f.LIU_LAMBDA,
        "alias_tolerance": run.ALIAS_TOLERANCE, "primary_contrast": list(run.PRIMARY),
        "practical_benefit": run.BENEFIT, "practical_harm": run.HARM,
        "bootstrap_draws": p1.BOOTSTRAP_DRAWS, "bootstrap_seed": p1.BOOTSTRAP_SEED,
        "runner_sha256": sha256_file(Path(run.__file__).resolve()), "labels_opened": False,
    }
    run.ROOT.mkdir(parents=True, exist_ok=True); atomic_write_json(run.REGISTRY, registry)
