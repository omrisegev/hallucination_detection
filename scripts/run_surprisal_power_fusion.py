"""Full six-arm answer-local surprisal-power comparison, using the frozen evaluator."""
import argparse
...
from spectral_utils.surprisal_power_fusion import METHODS, fit_all
...
NAMES={f'd{d}__{m}':f'Surprisal powers 1..{d}, incl chosen token / '+{'equal':'Normalized equal weights','iu':'IU-PCR'}[m]
       for d in (1,2,3) for m in ('equal','iu')}
...
def manifest_for(source,v2,temporal,varentropy):
    ...
    manifest.update(schema='surprisal-power-v1',methods=list(METHODS))
    files=[Path(__file__),ROOT/'spectral_utils/surprisal_power_fusion.py',ROOT/'spectral_utils/varentropy_contribution_fusion.py',
           ROOT/'scripts/run_varentropy_contribution_fusion.py',
           ROOT/'docs/experiments/SURPRISAL_POWER_FUSION_V1.md',
...
def evaluate(con,records,joined,temporal,v2,varentropy):
    ...
    primary={(f'd{d}__iu','d1__iu') for d in (2,3)}
    pairs=sorted(primary)+[(f'd{d}__equal','d1__equal') for d in (2,3)]+[(f'd{d}__iu',f'd{d}__equal') for d in (1,2,3)]
    ...
    for m,t in telemetry.items():
        W=np.stack(t.pop('weights')) if t['weights'] else np.zeros((0,48));k=16*int(m[1]);W=W[:,:k]
        ...
        weights[m]=dict(n=len(W),mean_standardized_coefficients=W.mean(axis=0).tolist() if len(W) else None,
            ...
            note='Both methods act on answer-standardized powers. Every16-column block ends with the chosen-token power.')
        t.pop('alphas',None)
    payload=dict(schema='surprisal-power-fusion-v1',n_answers=len(records),n_steps=int(joined['offsets'][-1]),
