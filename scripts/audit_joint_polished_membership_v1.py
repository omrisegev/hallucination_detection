...
from scripts.run_joint_polished_membership_v1 import OUT,inputs,KINDS,training_inputs,bank,references,PAIRS
...
def audit_mass_discovery(values,folds,stored):
    from spectral_utils.joint_polished_groups import discover_mass_groups
...
def audit_checkpoint(path,model,args):
    ...
    if not model['valid']:
        from spectral_utils.joint_polished_membership import fit_polished_joint
        replay=fit_polished_joint(*args,anchor_index=ANCHOR,seed=model['seed'])
...
def main():
    ...
    test=runpy.run_path(str(ROOT/'tests/test_joint_polished_refinement.py'));test.update(runpy.run_path(str(ROOT/'tests/test_joint_polished_groups.py')));test.update(runpy.run_path(str(ROOT/'tests/test_joint_mass_polish.py')));test.update(runpy.run_path(str(ROOT/'tests/test_joint_polished_integration.py')));passed=[]
    ...
    for kind in KINDS:
        ...
        for outer in range(5):
            ...
            np.testing.assert_allclose(expected,scores[kind+'__polished'][~rows],atol=1e-12,rtol=1e-12)
            max_score=max(max_score,float(np.max(np.abs(expected-scores[kind+'__polished'][~rows]))))
            print('MODEL_PASS',kind,outer,flush=True)
        new=scores[kind+'__polished'];old=scores['base__polished']
    ...
    for k,passed_margin in result['practical_preservation'].items():
        c=result['primary_contrasts'][k+'__polished minus base__polished']
        assert passed_margin==(native[k]==13769 and c['pb']['low']>-.01 and c['within']['low']>-.002)
    error=float(np.max(np.abs(scores['base__polished']-scores['base__old'])))
    assert error==result['baseline_replay_max_error']
    ci=result['primary_contrasts']['base__polished minus base__mass_reference']
