"""One frozen staged membership intervention on five full banks."""
import os
...
from spectral_utils.joint_staged_membership import fit_staged_joint,score_noise_aware_joint
...
OUT=ROOT/'results/joint_staged_membership_v1'
OLD=ROOT/'results/joint_sparse_refinement_v1'
HARD=ROOT/'results/joint_structured_stress_v1'
PREVIOUS=ROOT/'results/joint_signal_membership_v1'
KINDS=('base','duplicates','noise','near_copies','structured_noise')
PAIRS=[('noise__staged','noise__previous'),('base__staged','base__previous')]+[
    (k+'__staged','base__staged') for k in KINDS[1:]]
...
def references(data):
    ...
    with np.load(PREVIOUS/'SCORES.npz') as z:
        np.testing.assert_array_equal(z['gate'],data['gate'])
        for k in KINDS:scores[k+'__previous']=z[k+'__signal']
    return scores
...
def main():
    ...
    modules=('joint_staged_membership','joint_sparse_membership','joint_sparse_refinement',
    ...
    files=[Path(__file__),ROOT/'docs/experiments/JOINT_STAGED_MEMBERSHIP_V1.md',
    ...
    manifest=dict(schema='joint-staged-membership-v1',hashes={str(p):sha(p) for p in files})
    ...
    for kind in KINDS:
        x=bank(data,base,uids,kind);scores[kind+'__staged']=np.full(len(x),np.nan)
        ...
        for outer in range(5):
            ...
            if path.exists():meta=json.loads(path.read_text());model=meta['model']
                ...
                model=fit_staged_joint(*args,anchor_index=ANCHOR,seed=399170+outer,notify=notify)
                ...
                dump(path,meta)
            scores[kind+'__staged'][~rows]=score_noise_aware_joint(model,x[~rows],allow_anchor_fallback=True)
        ...
        if kind=='base':
            base_error=float(np.max(np.abs(scores['base__staged']-scores['base__old'])))
    ...
    preservation={k:bool(native[k]==13769 and ci[k+'__staged minus base__staged']['pb']['low']>-.01 and
        ci[k+'__staged minus base__staged']['within']['low']>-.002) for k in KINDS[1:]}
    result=dict(status='COMPLETE',metrics={k:scalar_metrics(m) for k,m in metrics.items()},
        ...
        fresh_fits=25,equivalent_replays=0,baseline_replay_max_error=base_error,baseline_preserved=bool(native['base']==13769 and base_error<=1e-11))
    dump(OUT/'RESULTS.json',result);np.savez_compressed(OUT/'SCORES.npz',**scores,gate=data['gate'])
    lines=['# Staged membership: full matched evaluation','','| Method | PB % | Within AUC |','|---|---:|---:|']
