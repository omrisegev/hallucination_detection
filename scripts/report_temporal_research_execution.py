...
def review_followup():
    folder=ROOT/'results/temporal_review_followup_v1'
    if not (folder/'DUFS_AUDIT.json').exists():return ''
    parts=['## עדכון בעקבות ביקורת Claude',
        'המשך התור העצבי הושהה באמצעות STOP_AFTER_JOB; העבודה שכבר התחילה מסיימת ושומרת תוצרים. אין פתיחת אימוני flow נוספים. התיקונים והאבחונים אינם מסקנת איכות על fold יחיד.',
    ...
    rows=[]
    for p in sorted(folder.glob('*_CHECKPOINT_AUDIT.json')):
        a=json.loads(p.read_text());s=a['summary'];ratio=100*s['grad_aux_sum']/s['grad_fm']
        rows.append(f"| {a['method']} / {a['bank']} | {s['fm']:.5f} | {s['repel']:.5f} | {s['curve']:.5f} | {100*s['repel_active']:.1f}% / {100*s['curve_active']:.1f}% | {ratio:.3f}% |")
    parts.append('| checkpoint | FM loss | repel | curve | hinge פעיל repel/curve | יחס נורמות גרדיאנט עזר/FM |\n|---|---:|---:|---:|---:|---:|\n'+'\n'.join(rows))
    parts.append('נבדקו 4,096 חלונות ולידציה ללא תוויות לכל checkpoint, ללא עדכון משקלים. היחס הוא מנה של נורמות ממוצעות לגרדיאנט העזר המשוקלל ול־FM. רכיבי העזר פעילים אך השפעתם קטנה בנקודה זו. ב־FM חושבו רכיבי העזר כאבחון בלבד; הם לא היו חלק מהאימון שלו. זו אינה היסטוריית loss משוחזרת.')
    parts.append('בקרות zero ושינוי סדר הן התערבויות בזמן ניקוד, ללא התאמה מחדש. zero משאיר את ה־innovation הנוכחי ואת המיקום והמסכות. מתאם DOT real/zero הוא כ־0.992 בשני ה־flows; מתאם כולל אינו מכריע לגבי שינוי בפסגות בתוך תשובה. TCN רגיש יותר להיסטוריה, ללא מסקנת איכות מכך.')
    position=ROOT/'results/temporal_position_control_v1'
    if (position/'METRICS.json').exists():
        package=json.loads((position/'METRICS.json').read_text());rows=[]
        for n,m in package['metrics'].items():rows.append(f"| {n} | {100*m['pb_all8']:.4f}% | {m['prm_within']:.6f} | {m['prmscore_q08']:.6f} |")
        parts+=['## בקרת המיקום הלא־לינארית — כל האוכלוסייה',
        ...
        contrast_rows=[]
        for name,c in package['contrasts'].items():
            if not c['primary']:continue
            lo,hi=c['pb_ci'];wl,wh=c['prm_within_ci']
            contrast_rows.append(f"| {name} | {100*c['pb_delta']:+.4f} | [{100*lo:+.4f}, {100*hi:+.4f}] | {c['prm_within_delta_common']:+.6f} | [{wl:+.6f}, {wh:+.6f}] |")
        parts+=['| השוואה ראשית | הפרש PB בנק׳ אחוז | CI 98.75% | הפרש within | CI 98.75% |\n|---|---:|---:|---:|---:|\n'+'\n'.join(contrast_rows),
...
'''
    text=text.replace('## הבסיס והבאגים',review_followup()+'## הבסיס והבאגים',1)
