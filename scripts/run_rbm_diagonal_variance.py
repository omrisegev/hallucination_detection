"""Learned conditional diagonal variance in the six-column RBM, using the frozen evaluator."""
import argparse
...
from spectral_utils.rbm_diagonal_variance import METHODS, fit_all, VARIANCE_PENALTY, VARIANCE_FLOOR
...
NAMES={'rbm':'Original RBM (100 iterations)','rbm_initial':'RBM before learning','rbm_continued':'Fixed variance RBM with matched extra training','rbm_diagonal':'Learned diagonal variance RBM'}
...
def worker(task):
    ...
    np.testing.assert_array_equal(S[:,METHODS.index('rbm')],moment_replay)
...
def manifest_for(source,v2,temporal,varentropy):
    ...
    manifest.update(schema='rbm-diagonal-variance-v1',methods=list(METHODS),regularization=VARIANCE_PENALTY,variance_floor=VARIANCE_FLOOR)
    files=[Path(__file__),ROOT/'spectral_utils/moment_rbm_fusion.py',ROOT/'spectral_utils/rbm_m3_powers.py', ROOT/'spectral_utils/rbm_diagonal_variance.py',ROOT/'spectral_utils/surprisal_power_fusion.py',ROOT/'spectral_utils/varentropy_contribution_fusion.py', ROOT/'spectral_utils/deem_adapter.py',ROOT/'spectral_utils/deem_b3_contract_ablation.py',ROOT/'spectral_utils/residual_graph_deem.py',
           ROOT/'scripts/run_varentropy_contribution_fusion.py',
           ROOT/'docs/experiments/RBM_DIAGONAL_VARIANCE_V1.md',
    ...
    files += [ROOT/'results/rbm_covariance_assumptions_v1/AUDIT.json',ROOT/'scripts/audit_rbm_covariance_assumptions.py',dependency_dir(source)/'RESULT_REVIEW.json',ROOT/'scripts/test_rbm_diagonal_variance.py']
...
def evaluate(con,records,joined,temporal,v2,varentropy):
    ...
    np.testing.assert_array_equal(scores['rbm'],scores['moment__rbm'])
    np.testing.assert_array_equal(scores['rbm_initial'],scores['moment__rbm_initial'])
    ...
    primary={('rbm_diagonal','rbm_continued')}
    pairs=[('rbm_diagonal','rbm_continued'),('rbm_diagonal','rbm'),('rbm_diagonal','rbm_initial'),('rbm_continued','rbm')]
    pairs += [('rbm_diagonal','ref__'+ref) for ref in ('entropy','k15__raw','k15__equal','k50__raw')]
    pairs += [('rbm_diagonal','shrinkage__rbm_shrinkage'),('rbm_diagonal','higher__d6__rbm')]
    ...
    for m,t in telemetry.items():
        ...
        covariance_keys=('variance_rmse','mean_rmse','covariance_relative_error','offdiag_relative_error')
        t['covariance']={k:np.quantile([v['covariance'][k] for v in diagnostics],[0,.25,.5,.75,1]).tolist() for k in covariance_keys}
        t['variance_floor_hits']=sum(v.get('variance_floor_hits',0) for v in diagnostics)
        weights[m]=dict(note='Standardized posterior coefficients orientation*w/D, not mean shifts w.',
            mean_oriented_weights=np.mean(np.asarray(saved)[:,:6],axis=0).tolist() if saved else [])
        if diagnostics and 'conditional_variance' in diagnostics[0]:
            t['conditional_variance_quantiles']=np.quantile(np.concatenate([v['conditional_variance'] for v in diagnostics]),[0,.25,.5,.75,1]).tolist()
    payload=dict(schema='rbm-diagonal-variance-v1',n_answers=len(records),n_steps=int(joined['offsets'][-1]),
...
def main():
    ...
    args=p.parse_args();audit=json.loads((ROOT/'results/rbm_covariance_assumptions_v1/AUDIT.json').read_text());assert audit['status']=='COMPLETE' and audit['n_answers']==13769
