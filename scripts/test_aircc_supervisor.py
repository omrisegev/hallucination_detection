...
def run():
    ...
    supervisor = Path(__file__).with_name('complete_answer_position_fusion.py')
    with tempfile.TemporaryDirectory() as tmp:
        root=Path(tmp);out=root/'results/answer_position_fusion_v1'
        (root/'scripts').mkdir();(out/'smoke').mkdir(parents=True)
        (root/'scripts/run_answer_position_fusion.py').write_text(
            'import os,time\nfrom pathlib import Path\n'
            'out=Path(__file__).resolve().parents[1]/"results/answer_position_fusion_v1/smoke"\n'
        ...
        loader=('import importlib.util,sys\nfrom pathlib import Path\n'
                ...
                'm.ROOT=Path(sys.argv[2]);m.OUT=m.ROOT/"results/answer_position_fusion_v1"\n'
                'sys.argv=["supervisor","--source-root",sys.argv[2]];m.main()\n')
