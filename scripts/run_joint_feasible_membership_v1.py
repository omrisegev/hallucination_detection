"""One frozen feasible refinement intervention on five full banks."""
import os
...
from spectral_utils.joint_feasible_membership import fit_feasible_joint,score_noise_aware_joint
...
PAIRS=[('near_copies__feasible','near_copies__regroup_reference'),('base__feasible','base__previous')]+[
    (k+'__feasible','base__feasible') for k in KINDS[1:]]
...
def main():
    ...
    modules=('joint_feasible_membership','joint_feasible_refinement','joint_staged_membership','joint_sparse_membership','joint_sparse_refinement',
    ...
    files=[Path(__file__),ROOT/'docs/experiments/JOINT_FEASIBLE_REFINEMENT_V1.md',
        *[ROOT/f'spectral_utils/{m}.py' for m in modules],
        *[p/name for p in (OLD,HARD,PREVIOUS,ROBUST) for name in ('SCORES.npz','MANIFEST.json','AUDIT.json')]]
    manifest=dict(schema='joint-feasible-membership-v1',hashes={str(p):sha(p) for p in files})
    ...
    for kind in KINDS:
        x=bank(data,base,uids,kind);scores[kind+'__feasible']=np.full(len(x),np.nan)
        ...
        for outer in range(5):
            ...
            if path.exists():meta=json.loads(path.read_text());model=meta['model']
                ...
                model=fit_feasible_joint(*args,anchor_index=ANCHOR,seed=399170+outer,notify=notify)
                ...
                dump(path,meta)
            scores[kind+'__feasible'][~rows]=score_noise_aware_joint(model,x[~rows],allow_anchor_fallback=True)
        ...
        if kind=='base':
            base_error=float(np.max(np.abs(scores['base__feasible']-scores['base__old'])))
    ...
    preservation={k:bool(native[k]==13769 and ci[k+'__feasible minus base__feasible']['pb']['low']>-.01 and
        ci[k+'__feasible minus base__feasible']['within']['low']>-.002) for k in KINDS[1:]}
    result=dict(status='COMPLETE',metrics={k:scalar_metrics(m) for k,m in metrics.items()},
        ...
        fresh_fits=25,equivalent_replays=0,baseline_replay_max_error=base_error,baseline_preserved=bool(native['base']==13769 and ci['base__feasible minus base__previous']['pb']['low']>-.01 and ci['base__feasible minus base__previous']['within']['low']>-.002))
    dump(OUT/'RESULTS.json',result);np.savez_compressed(OUT/'SCORES.npz',**scores,gate=data['gate'])
    lines=['# Information-feasible refinement: full matched evaluation','','| Method | PB % | Within AUC |','|---|---:|---:|']
