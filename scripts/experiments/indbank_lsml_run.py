"""indbank_lsml_prmbench_v1: bank11 + the weak, entropy-INDEPENDENT channels (label-free screen) under the
bank11 CONT L-SML recipe, PRMBench first.  Frozen protocol: results/indbank_lsml_prmbench_v1/PROTOCOL.json.
...
STAGE = ROOT / 'results/indbank_lsml_prmbench_v1'; OUT = STAGE / RUN_ID; OUT.mkdir(parents=True, exist_ok=True)
...
INPUTS = {'level_bank': TPF / 'DERIVATIVE_CHANNELS.npz', 'ct7_tokens': MAIN / 'results/ct7_token_lsml_v1/CT7_TOKEN_MATRICES.npz',
          ...
          'baseline_scores': DEPTH / 'results/step_level_bank_baseline_v1/STEP_SCORES.npz', 'pool_z': SCR / 'pool_z.npy', 'pool_names': SCR / 'pool_names.json', 'pool_structure': STAGE / 'POOL_STRUCTURE.csv', 'hist29_profiles': SCR / 'hist29_top10_profiles.npy', 'folds_v2': MAIN / 'results/localization_source_group_audit_v1/FOLDS_V2.json'}
...
# ------------------------------------------------------------------ channels: the pool (already answer-z) + label-free IND screen
t = time.perf_counter()
POOL = np.load(INPUTS['pool_z']); PN = json.loads(INPUTS['pool_names'].read_text(encoding='utf8')); assert POOL.shape == (int(off[-1]), len(PN))
S = pd.read_csv(INPUTS['pool_structure']).set_index('channel')
names11 = PN[:11]; assert names11 == list(np.load(INPUTS['level_bank'])['channels'].astype(str))
IND = [c for c in PN[11:] if (not S.loc[c, 'in_bank11']) and abs(S.loc[c, 'r_level_marginal']) < 0.35 and S.loc[c, 'max_r_with_bank11'] < 0.8 and S.loc[c, 'sd_after_z'] > 0.5]
assert IND == P['selected_IND_21'], (IND, P['selected_IND_21'])
lf_sign = {c: (float(np.sign(S.loc[c, 'r_level_marginal'])) or 1.0) for c in IND}                     # label-free orientation
or_sign = {c: (1.0 if S.loc[c, 'within_auc_single'] >= 0.5 else -1.0) for c in IND}                   # oracle orientation (labels; diagnostic)
Xind = POOL[:, [PN.index(c) for c in IND]]
raw = np.column_stack([POOL[:, :11], Xind * np.array([lf_sign[c] for c in IND]), Xind * np.array([or_sign[c] for c in IND])])
names = names11 + ['lf__' + c for c in IND] + ['or__' + c for c in IND]; assert np.isfinite(raw).all()
missing = {}
values = answer_standardize(raw, off)                                                                   # idempotent on z-scores; keeps the recipe call
BANKS = {'B11': list(range(11)), 'B11_IND_lf': list(range(32)), 'B11_IND_or': list(range(11)) + list(range(32, 53))}
orient_agree = int(sum(lf_sign[c] == or_sign[c] for c in IND))
timing['channels_s'] = time.perf_counter() - t
dump(OUT / 'INPUT_MANIFEST.json', {k: {'path': str(v), 'bytes': v.stat().st_size, 'sha256': sha(v)} for k, v in INPUTS.items()} | {'channels': names, 'IND': IND, 'lf_sign': lf_sign, 'oracle_sign': or_sign, 'orientation_agreement': orient_agree, 'missing_step_values_filled': missing, 'population': {'answers': n, 'prm': int(prm.sum()), 'eligible': int(eligible.sum()), 'noncontrol': int(noncontrol.sum()), 'steps': int(off[-1])}})
snap = OUT / 'SOURCE_SNAPSHOT'; code = {}
for rel, base in [('scripts/experiments/indbank_lsml_run.py', ROOT), ('spectral_utils/lsml_gate_locator_research.py', DEPTH), ('spectral_utils/fusion_utils.py', DEPTH)]:
...
print(f'channels ready in {timing["channels_s"]:.0f}s; IND={len(IND)}; lf/oracle orientation agree on {orient_agree}/{len(IND)}', flush=True)
...
diag['replay'] = rep; diag['calibration_thresholds'] = cal_thr; diag['IND'] = IND; diag['lf_sign'] = lf_sign; diag['oracle_sign'] = or_sign; diag['orientation_agreement'] = orient_agree
diag['ind_block_weight_per_fold'] = {b: [float(np.sum(f['weights'][11:])) for f in sorted([f for f in fit_log if f['bank'] == b], key=lambda f: f['fold'])] for b in ['B11_IND_lf', 'B11_IND_or']}
...
prim = [('B11_IND_lf_lsml', 'B11_lsml'), ('B11_IND_lf_lsml', 'B11_IND_lf_equal')]
sec = [('B11_IND_or_lsml', 'B11_lsml'), ('B11_IND_or_lsml', 'B11_IND_or_equal'), ('B11_IND_or_lsml', 'B11_IND_lf_lsml'), ('B11_IND_lf_lsml', 'B11_IND_lf_group_equal'), ('B11_IND_or_lsml', 'B11_IND_or_group_equal'),
       ('B11_lsml', 'B11_equal'), ('B11_lsml', 'B11_group_equal'), ('B11_IND_lf_equal', 'B11_equal'), ('B11_IND_or_equal', 'B11_equal'), ('B11_lsml', 'ct7'), ('B11_IND_lf_lsml', 'ct7'), ('B11_IND_or_lsml', 'ct7'), ('B11_lsml_replay4', 'ct7')]
K = 4; rows = []; deltas = {}
for a, b in prim + sec:
