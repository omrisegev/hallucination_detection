"""Run frozen selected-token and residual-tail probability fusion.

Track A fits selected+tail probability fusion inside each single answer for PB/PRMB
localization. Track B aggregates the 17 token inputs per answer and fits across answers
...
from spectral_utils.direct_probability_fusion_v2 import (  # noqa: E402
...
METHODS = ("entropy", "augmented_equal", "augmented_iu", "augmented_joint_lw")
FIT_METHOD = {
    "augmented_equal": "equal",
    "augmented_iu": "iu",
    "augmented_joint_lw": "joint_lw",
}
BOOT_SEED = 2026091017
...
def validate_frozen_inputs(k: int) -> dict[str, Any]:
    ...
    if not audit_path.exists():
        raise FileNotFoundError(
            "Run audit_direct_probability_data_v2.py in this worktree before scoring"
        )
    ...
    required = {
        ...
        "v1_localization_metrics": ROOT / "results" / "direct_probability_fusion_v1" / "LOCALIZATION.json",
        "v1_localization_scores": ROOT / "results" / "direct_probability_fusion_v1" / "LOCALIZATION_SCORES.npz",
        "v1_historical_metrics": ROOT / "results" / "direct_probability_fusion_v1" / "HISTORICAL_24.json",
        "v1_historical_scores": ROOT / "results" / "direct_probability_fusion_v1" / "HISTORICAL_24_SCORES.npz",
        "v1_localization_manifest": ROOT / "results" / "direct_probability_fusion_v1" / "RUN_MANIFEST_LOCALIZATION.json",
        "v1_historical_manifest": ROOT / "results" / "direct_probability_fusion_v1" / "RUN_MANIFEST_HISTORICAL.json",
        ...
        "input_freeze": (
            ...
            / "docs"
            / "experiments"
            / "DIRECT_PROBABILITY_FUSION_V2_SELECTED_TAIL_INPUT_FREEZE.json"
        ),
    }
    ...
    for relative, expected in freeze["local_comparator_files"].items():
        path = ROOT / Path(relative)
        actual = sha256_file(path)
        if actual != expected:
            raise ValueError(f"frozen v1 comparator changed: {relative}")
        frozen_hashes[relative] = actual
...
def _local_token_scores(
    ...
    risk = augmented_probability_risk(
        _topk_payload(row), row["token_spilled_energies"], k=k
    )
...
def _localization_metrics(
    ...
    comparisons = (
        ("augmented_joint_lw", "entropy"),
        ("augmented_iu", "entropy"),
    )
    pb_only_comparisons = (
        ("augmented_iu", MIND_GAP_COMPARATOR),
        ("augmented_joint_lw", MIND_GAP_COMPARATOR),
    )
...
def run_localization(k: int, bootstrap: int) -> dict[str, Any]:
    ...
    payload = {
        "schema": "direct-probability-localization-v2-selected-tail",
        "k": k,
        "matrix_columns": list(augmented_feature_names(k)),
...
def run_historical(k: int, bootstrap: int) -> dict[str, Any]:
    ...
    for position, cell in enumerate(INSCOPE, start=1):
        ...
        for candidate in candidates:
            ...
            rank_risk = augmented_probability_risk(
                _topk_payload(candidate), candidate["token_spilled_energies"], k=k
            )
        ...
        cells_out[cell] = {
            ...
            "augmented_iu_minus_historical_iu_pcr_ci97_5": np.percentile(
                cell_bootstrap, [1.25, 98.75]
            ).tolist(),
            "augmented_joint_lw_minus_historical_iu_pcr": float(
                method_auc["augmented_joint_lw"] - reference_iu_pcr
            ),
    ...
    comparisons = (
        ("augmented_joint_lw", "entropy"),
        ("augmented_iu", "entropy"),
        ("augmented_joint_lw", "augmented_iu"),
        ("augmented_iu", "historical_iu_pcr"),
        ("augmented_joint_lw", "historical_iu_pcr"),
    )
    vectors = {
        name: np.asarray(
            [
    ...
    primary_key = "augmented_iu_minus_historical_iu_pcr"
    ...
    result = {
        "schema": "direct-probability-historical24-v2-selected-tail",
        "k": k,
        "matrix_columns": list(augmented_feature_names(k)),
        "answer_aggregation": "top-10 token mean separately for each of 15 ranks, selected surprisal, and residual tail",
        "fit_population": (
...
def main() -> None:
    ...
    if args.k != DEFAULT_K:
        raise ValueError("v2 is frozen to K=15; no K search is allowed")
    ...
    manifest = {
        "schema": "direct-probability-fusion-run-v2-selected-tail",
        "stage": args.stage,
        "k": args.k,
        "bootstrap": args.bootstrap,
        "elapsed_seconds": time.time() - started,
        "protocol": "docs/experiments/DIRECT_PROBABILITY_FUSION_V2_SELECTED_TAIL.md",
        "input_contract": input_contract,
        "source_sha256": {
            "core_v1": sha256_file(ROOT / "spectral_utils" / "direct_probability_fusion.py"),
            "core_v2": sha256_file(ROOT / "spectral_utils" / "direct_probability_fusion_v2.py"),
            "driver": sha256_file(Path(__file__)),
            "protocol": sha256_file(ROOT / "docs" / "experiments" / "DIRECT_PROBABILITY_FUSION_V2_SELECTED_TAIL.md"),
        },
    }
