"""Renyi-view fusion v1 driver (Stage 3 prototype), on the frozen localization evaluator.

Smoke = 27-answer mechanics/feasibility check only (3 answers per cell by
token-length quantile).  The full 13,769-answer run and the final design are
DEFERRED until the cross-rank experiment is reviewed; this driver must not be
launched on the full population before that review.
"""
import argparse
...
from spectral_utils.renyi_view_fusion import (METHODS, NOT_APPLICABLE, MAX_WEIGHTS, COLUMN_NAMES, DIAG_NAMES,
    VIEW_NAMES, SINGLE_VIEWS, FUSED_METHODS, ALPHAS, fit_all, view_diagnostics)
...
OUT = ROOT / 'results/renyi_view_fusion_v1'
MEMORY_GUARD_KB = 3_000_000
VIEW_LABEL = {'H0.5': 'Renyi H_0.5', 'H1': 'Shannon H_1 (entropy15)', 'H2': 'Renyi H_2 (collision)',
              'H4': 'Renyi H_4', 'Hinf': 'Min-entropy H_inf (= s_1)', 'sel1': 'Selected-token surprisal'}
NAMES = {'view__' + v: 'Single view / ' + VIEW_LABEL[v] for v in SINGLE_VIEWS}
NAMES.update({m: {'R5': 'Five Renyi views', 'R5_sel': 'Five Renyi views + SEL'}[m.split('__')[0]] + ' / ' +
              {'equal': 'oriented equal weights', 'iu': 'IU-PCR learned weights',
               'shrink': 'joint-target LW shrinkage IU'}[m.split('__')[1]] for m in FUSED_METHODS})
...
PRIMARY = {('R5_sel__iu', 'R5_sel__equal'), ('R5_sel__iu', 'view__H1')}
...
def contrast_pairs():
    ...
    pairs += [('R5__iu', 'R5__equal'), ('R5_sel__iu', 'R5_sel__equal'), ('R5_sel__shrink', 'R5_sel__equal'),
              ('R5_sel__shrink', 'R5_sel__iu'), ('R5_sel__iu', 'R5__iu'), ('R5_sel__equal', 'R5__equal')]
...
def worker(task):
    ...
    fits, failures, seconds = fit_all(lp, chosen)
...
def manifest_for(source, v2, temporal, varentropy):
    ...
    manifest.update(schema='renyi-view-fusion-v1', methods=list(METHODS), not_applicable=NOT_APPLICABLE,
                    ...
                    stage='PROTOTYPE: final design deferred until the cross-rank review')
    files = [Path(__file__), ROOT / 'spectral_utils/renyi_view_fusion.py', ROOT / 'scripts/test_renyi_view_fusion.py',
             ROOT / 'spectral_utils/varentropy_contribution_fusion.py', ROOT / 'spectral_utils/fixed_gate_readout.py',
             ROOT / 'docs/experiments/RENYI_VIEW_FUSION_V1_DRAFT.md',
...
def aggregate_diagnostics(infos, scope):
    ...
    for m in FUSED_METHODS:
        ...
        fused[m] = dict(n_fitted=len(ds), n_failed=sum(m in r['failures'] for r in infos),
                        ...
                        g2_hat=_nanstats([d.get('g2_hat') for d in ds]) if not m.endswith('equal') else None)
    return dict(scope=scope, n_answers=n, columns=cols, note='Rows/columns of the matrices follow `columns`. '
                ...
                condition_R5=_nanstats([d['condition_R5'] for d in views]),
                condition_R5_sel=_nanstats([d['condition_R5_sel'] for d in views]),
                fraction_condition_R5_sel_above_1e6=float(np.mean([(d['condition_R5_sel'] or 0) > 1e6 for d in views])),
...
def evaluate(con, records, joined, temporal, varentropy):
    ...
    print('[evaluate] frozen references reproduced; bootstrap 10000', flush=True)
    ...
    for m, t in telemetry.items():
        ...
        t.pop('alphas', None)
    payload = dict(schema='renyi-view-fusion-v1', n_answers=len(records), n_steps=int(joined['offsets'][-1]),
                   scope='Full cached localization development; fusion answer-local, gate/calibration external; '
                         'no historical24 result. PROTOTYPE evaluation; final design deferred to the cross-rank review.',
...
def main():
    ...
    p.add_argument('--allow-full', action='store_true', help='required for a non-smoke run; the full run is deferred')
