...
from scripts.run_joint_signal_membership_v1 import OUT,inputs,KINDS,training_inputs,bank,references,PAIRS
...
def audit_model(model,x,offsets,sources):
    ...
    if membership:
        ...
        for r in membership['rounds']:
            ...
            labs=np.asarray(r['discovery']['labels']);signal_groups=[]
            ...
            detail=r['sparse']['signal_membership']
...
def independent_intervals(data,scores,gate):
...
def replay_empty_failure(model,x,offsets,sources,folds):
    """Recover diagnostics lost when the frozen learner raised before return.

    This is an explicit diagnostic refit, not a repaired candidate or retry used
    for scores. Then audit_model independently checks its sparse algebra.
    """
    from spectral_utils.joint_sparse_membership import source_grams,calibrate_penalty,sparse_membership
    ...
    assert model['failure']=='ValueError: TOO_FEW_ACTIVE_MEASUREMENTS'
    z=alias_coordinates(x,model['aliases']);seed=model['seed'];cov=covariance_matrix(z)
    discovery=discover_sourcefold_groups(z,np.repeat(folds,np.diff(offsets)),seed=seed+100)
    grams,sizes,_=source_grams(z,offsets,sources,folds)
    penalty,cal=calibrate_penalty(grams,int(sizes.sum()),cov,np.asarray(discovery['labels']),seed=seed+5000)
    local,sparse=sparse_membership(cov,np.asarray(discovery['labels']),penalty=penalty,seed=seed+200)
    labs=np.asarray(discovery['labels']);starts=[s for s in sparse['starts'] if s['converged']]
    global_support=np.flatnonzero(np.any([s['v']!=0 for s in starts],axis=0))
    groups=np.unique(labs[global_support]);active=local[np.isin(labs[local],groups)]
    assert len(active)<6
    sparse['signal_membership']=dict(signal_groups=groups,nuisance_groups=np.setdiff1d(np.unique(labs),groups),
        retained=active,global_support_union=global_support)
    r=dict(round=0,active_before=np.arange(z.shape[1]),active_after=active,
        discovery=discovery,calibration=cal,sparse=sparse)
    diagnostic=dict(model, membership=dict(rounds=[r],active=active,valid=False))
    error,rounds=audit_model(diagnostic,x,offsets,sources)
    dump(OUT/f"FAILURE_REPLAY_seed{seed}.json",dict(scope='diagnostic only; original failed scores remain frozen',
        model=diagnostic,independent_audit='PASS'))
    return error,rounds
...
def audit_checkpoint(path,model,args):
    ...
    if not model['valid'] and 'membership' not in model:error,n=replay_empty_failure(model,*args)
    else:error,n=audit_model(model,args[0],args[1],args[2])
...
def main():
    ...
    test=runpy.run_path(str(ROOT/'tests/test_joint_signal_membership.py'));passed=[]
    ...
    for kind in KINDS:
        ...
        for outer in range(5):
            ...
            np.testing.assert_allclose(expected,scores[kind+'__signal'][~rows],atol=1e-12,rtol=1e-12)
            max_score=max(max_score,float(np.max(np.abs(expected-scores[kind+'__signal'][~rows]))))
            print('MODEL_PASS',kind,outer,flush=True)
        new=scores[kind+'__signal'];old=scores['base__signal']
    ...
    for k,passed_margin in result['practical_preservation'].items():
        c=result['primary_contrasts'][k+'__signal minus base__signal']
        assert passed_margin==(native[k]==13769 and c['pb']['low']>-.01 and c['within']['low']>-.002)
    error=float(np.max(np.abs(scores['base__signal']-scores['base__old'])))
    ...
    audit=dict(status='PASS',answers=len(uids),steps=len(base),metrics_checked=len(scores),
        ...
        'global-group support from converged starts','null covariance and sparse objective',
