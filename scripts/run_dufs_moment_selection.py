"""Answer-local DUFS moment selection before RBM, using the frozen evaluator."""
import argparse
...
from spectral_utils.dufs_moment_selection import METHODS, fit_all, BANKS
...
BANK_NAMES={'all12':'All 12 moments','original6':'Original 6 moments','dufs6':'Adapted DUFS: 6 selected moments','correlation6':'Low correlation: 6 selected moments'}
NAMES={b+'__'+solver: name+' / '+('trained RBM' if solver=='rbm' else 'RBM before learning') for b,name in BANK_NAMES.items() for solver in ('rbm','rbm_initial')}
...
def reference_dirs(source):
    return {'moment':source/'.worktrees/moment-rbm-fusion-v1/results/moment_rbm_fusion_v1_staged/fast',
            ...
            'shrinkage':source/'.worktrees/rbm-weight-shrinkage-v1/results/rbm_weight_shrinkage_v1',
            'higher':source/'.worktrees/higher-moment-fusion-v1/results/higher_moment_fusion_v1'}
...
def worker(task):
    ...
    np.testing.assert_array_equal(S[:,METHODS.index('original6__rbm')],moment_replay)
...
def manifest_for(source,v2,temporal,varentropy):
    ...
    manifest.update(schema='dufs-moment-selection-v1',methods=list(METHODS),selector=dict(epochs=120,seeds=[0,1,2],keep=6,correlation='greedy squared Pearson; least total redundancy first; stable index ties'))
    files=[Path(__file__),ROOT/'spectral_utils/moment_rbm_fusion.py',ROOT/'spectral_utils/rbm_m3_powers.py', ROOT/'spectral_utils/dufs_moment_selection.py',ROOT/'spectral_utils/adapted_dufs.py',ROOT/'spectral_utils/higher_moment_fusion.py',ROOT/'spectral_utils/surprisal_power_fusion.py',ROOT/'spectral_utils/varentropy_contribution_fusion.py', ROOT/'spectral_utils/deem_adapter.py',ROOT/'spectral_utils/deem_b3_contract_ablation.py',ROOT/'spectral_utils/residual_graph_deem.py',
           ROOT/'scripts/run_varentropy_contribution_fusion.py',
           ROOT/'docs/experiments/DUFS_MOMENT_SELECTION_V1.md',
    ...
    files += [dependency_dir(source)/'RESULT_REVIEW.json',ROOT/'scripts/test_dufs_moment_selection.py',
              ROOT/'scripts/review_dufs_moment_selection.py',ROOT/'scripts/finish_dufs_moment_selection.py',
              ROOT/'scripts/run_dufs_moment_selection_pipeline.py',ROOT/'scripts/verify_direct_probability_temporal_results.py']
...
def evaluate(con,records,joined,temporal,v2,varentropy):
    ...
    for family,directory in reference_dirs(base.old.SOURCE_ROOT).items():
        ...
        with np.load(directory/'SCORES.npz') as z:
            for m,x in d['metrics'].items():
                ...
                if family=='shrinkage' and m!='rbm_shrinkage':continue
                if family=='higher' and m not in ('d6__rbm','d6__rbm_initial'):continue
                ...
                elif family=='shrinkage':NAMES[key]='RBM with weight shrinkage (saved)'
                elif family=='higher':NAMES[key]='Order6 '+('RBM before learning' if m.endswith('initial') else 'trained RBM')+' (saved)'
                else:
    ...
    for solver in ('rbm','rbm_initial'):
        np.testing.assert_array_equal(scores['original6__'+solver],scores['moment__'+solver])
        np.testing.assert_array_equal(scores['all12__'+solver],scores['higher__d6__'+solver])
    ...
    primary={('dufs6__rbm','correlation6__rbm'),('dufs6__rbm','all12__rbm')}
    pairs=sorted(primary)+[('dufs6__rbm','original6__rbm')]
    pairs += [(b+'__rbm',b+'__rbm_initial') for b in BANKS]
    pairs += [('dufs6__rbm','ref__'+ref) for ref in ('entropy','k15__raw','k15__equal','k50__raw')]
    ...
    for m,t in telemetry.items():
        ...
        for d in diagnostics:count[d['columns']]+=1
        t['selected_column_counts']=count.tolist()
        t['selection_seconds_total']=sum(d['selection_seconds'] for d in diagnostics) if m.endswith('__rbm_initial') else 0.
        if m.startswith('dufs6'):
            t['seed_top6_jaccard_quantiles']=np.quantile([v for d in diagnostics for v in d['selection']['seed_top6_jaccard']],[0,.25,.5,.75,1]).tolist()
            t['mean_gate_probability']=np.mean([d['selection']['raw_probabilities'] for d in diagnostics],axis=0).tolist()
        weights[m]=dict(note='Oriented standardized RBM coefficients; unselected columns are zero.',feature_names=feature_names(6),
            mean_oriented_weights=np.mean(np.asarray(saved)[:,:12],axis=0).tolist() if saved else [])
    payload=dict(schema='dufs-moment-selection-v1',n_answers=len(records),n_steps=int(joined['offsets'][-1]),
...
def main():
    ...
    args=p.parse_args()
    base.old.configure_source_root(args.source_root.resolve());verify_dependency(args.source_root.resolve());OUT.mkdir(parents=True,exist_ok=True)
