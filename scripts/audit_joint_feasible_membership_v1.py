...
from scripts.run_joint_feasible_membership_v1 import OUT,inputs,KINDS,training_inputs,bank,references,PAIRS
...
def main():
    ...
    test=runpy.run_path(str(ROOT/'tests/test_joint_feasible_refinement.py'));passed=[]
    ...
    for kind in KINDS:
        ...
        for outer in range(5):
            ...
            np.testing.assert_allclose(expected,scores[kind+'__feasible'][~rows],atol=1e-12,rtol=1e-12)
            max_score=max(max_score,float(np.max(np.abs(expected-scores[kind+'__feasible'][~rows]))))
            print('MODEL_PASS',kind,outer,flush=True)
        new=scores[kind+'__feasible'];old=scores['base__feasible']
    ...
    for k,passed_margin in result['practical_preservation'].items():
        c=result['primary_contrasts'][k+'__feasible minus base__feasible']
        assert passed_margin==(native[k]==13769 and c['pb']['low']>-.01 and c['within']['low']>-.002)
    error=float(np.max(np.abs(scores['base__feasible']-scores['base__old'])))
    assert error==result['baseline_replay_max_error']
    ci=result['primary_contrasts']['base__feasible minus base__previous']
