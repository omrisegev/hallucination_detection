"""RBM m3 ablation and raw surprisal-power transfer, using the frozen evaluator."""
import argparse
...
from spectral_utils.rbm_m3_powers import METHODS, fit_all
...
OUT = ROOT / 'results/rbm_m3_powers_v1'
NAMES={'mom6_rbm':'Original six-moment RBM replay','mom6_shared_rbm':'Six-moment RBM / shared direction','mom5_rbm':'RBM without distribution m3','power48_rbm':'Raw surprisal powers / RBM','power48_initial':'Raw surprisal powers / RBM before learning'}
...
def worker(task):
    ...
    np.testing.assert_array_equal(S[:,METHODS.index('mom6_rbm')],moment_replay)
...
def manifest_for(source,v2,temporal,varentropy):
    ...
    manifest.update(schema='rbm-m3-powers-v1',methods=list(METHODS))
    files=[Path(__file__),ROOT/'spectral_utils/moment_rbm_fusion.py',ROOT/'spectral_utils/rbm_m3_powers.py',ROOT/'spectral_utils/surprisal_power_fusion.py',ROOT/'spectral_utils/varentropy_contribution_fusion.py', ROOT/'spectral_utils/deem_adapter.py',ROOT/'spectral_utils/deem_b3_contract_ablation.py',ROOT/'spectral_utils/residual_graph_deem.py',
           ROOT/'scripts/run_varentropy_contribution_fusion.py',
           ROOT/'docs/experiments/RBM_M3_POWERS_V1.md',
...
def evaluate(con,records,joined,temporal,v2,varentropy):
    ...
    primary={('mom6_shared_rbm','mom5_rbm'),('power48_rbm','power__d3__iu')}
    pairs=sorted(primary)+[('mom6_shared_rbm','mom6_rbm'),('power48_rbm','power48_initial'),('power48_rbm','power__d3__equal'),('power48_rbm','mom6_rbm')]
    ...
    payload=dict(schema='rbm-m3-powers-v1',n_answers=len(records),n_steps=int(joined['offsets'][-1]),
