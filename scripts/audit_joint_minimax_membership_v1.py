...
from scripts.run_joint_minimax_membership_v1 import OUT,inputs,KINDS,training_inputs,bank,references,PAIRS
...
def audit_regroup_path(model,x,offsets,folds):
    ...
    for step,state in enumerate(path):
        ...
        # Direct solves for EVERY possible deletion independently check the
        # production inverse/conditional-loss ranking.
        importance=[]
        for index in range(len(ids)):
            proposal=np.delete(ids,index)
            after=factor_information(c[np.ix_(proposal,proposal)],reference_a[proposal])
            importance.append(float(np.max(1-after[relevant]/reference_info[relevant])))
        ...
        for attempt,index in zip(attempts,candidates):
            ...
            replay=discover_sourcefold_groups(values[:,proposal],rowfold,seed=sel['group_seed'])
            ...
            if replay['status']=='SELECTED':
                ...
                np.testing.assert_allclose(replay['ari'],stored['ari'],atol=1e-12)
            if 'reason' in attempt:
                ...
                np.testing.assert_allclose(importance[index],attempt['worst_initial_information_loss'],atol=1e-9,rtol=1e-7)
...
def main():
    ...
    test=runpy.run_path(str(ROOT/'tests/test_joint_minimax_refinement.py'));passed=[]
    ...
    for kind in KINDS:
        ...
        for outer in range(5):
            ...
            np.testing.assert_allclose(expected,scores[kind+'__minimax'][~rows],atol=1e-12,rtol=1e-12)
            max_score=max(max_score,float(np.max(np.abs(expected-scores[kind+'__minimax'][~rows]))))
            print('MODEL_PASS',kind,outer,flush=True)
        new=scores[kind+'__minimax'];old=scores['base__minimax']
    ...
    for k,passed_margin in result['practical_preservation'].items():
        c=result['primary_contrasts'][k+'__minimax minus base__minimax']
        assert passed_margin==(native[k]==13769 and c['pb']['low']>-.01 and c['within']['low']>-.002)
    error=float(np.max(np.abs(scores['base__minimax']-scores['base__old'])))
    assert error==result['baseline_replay_max_error']
    ci=result['primary_contrasts']['base__minimax minus base__previous']
    ...
    audit=dict(status='PASS',answers=len(uids),steps=len(base),metrics_checked=len(scores),
        ...
        'full information-refinement path and direct-solve minimax ranking','final factor covariance and weights',
