#!/usr/bin/env python3
"""Evaluate the five-seed structured input-adapter finalist."""
...
ARMS=("D1_BASE","J2_BLOCK_PLUS_CROSS_ADAM_R2"); SEEDS=(0,1,2,3,4)
def main():
 run_dir=ROOT/"local_cache/deem_b3_moe_v1/input_adapter_adam_full"; frozen_dir=ROOT/"local_cache/deem_b3_moe_v1/b3_frozen"; bundle_dir=ROOT/"local_cache/deem_b3_moe_v1/bundles"; sidecar_dir=ROOT/"local_cache/deem_b3_moe_v1/label_sidecars"; out_dir=ROOT/"local_cache/deem_b3_moe_v1/input_adapter_adam_full_eval"; out_dir.mkdir(parents=True,exist_ok=True)
 ...
 assert definition["source_hashes"]["runner"]==sha256_file(ROOT/"scripts/run_deem_b3_input_adapter_adam_full.py"); assert len(freeze["records"])==240 and not freeze["labels_accessed_during_fit"]
 records={(row["cell_id"],row["arm"],row["seed"]):row for row in freeze["records"]}; bundles={}; scores={}; health=[]
 for row in registry["cells"]:
  ...
  scores[cell]["B3_FROZEN"]=np.mean(np.stack(frozen),axis=0)
  for arm in ARMS:
   values=[]
   for seed in SEEDS:
    record=records[(cell,arm,seed)]; path=run_dir/record["npz"]; assert sha256_file(path)==record["npz_sha256"]; meta=json.loads((run_dir/record["json"]).read_text()); copy=dict(meta); expected=copy.pop("content_sha256"); assert canonical_sha256(copy)==expected
    with np.load(path,allow_pickle=False) as data: assert tuple(str(x) for x in data["row_id"].tolist())==bundle.row_ids; values.append(np.asarray(data["score"]))
    health.append({"cell_id":cell,"arm":arm,"seed":seed,**meta["health"]})
   scores[cell][arm]=np.mean(np.stack(values),axis=0)
 pre={"schema":"input_adapter_adam_full_pre_label","all_240_fits_verified":True,"labels_imported":False}; pre["content_sha256"]=canonical_sha256(pre); atomic_write_json(out_dir/"PRE_LABEL_FREEZE.json",pre); labels=importlib.import_module("spectral_utils.residual_graph_deem_labels")
 metrics=[]
 for cell,bundle in bundles.items():
  y=labels.join_labels_by_id(bundle,labels.load_label_sidecar(sidecar_dir/f"{cell}.npz"))
  for arm in ("B3_FROZEN",)+ARMS: metrics.append({"cell_id":cell,"dataset_family":bundle.dataset_family,"arm":arm,"auroc":float(roc_auc_score(y,scores[cell][arm])),"auprc":float(average_precision_score(y,scores[cell][arm]))})
 lookup={(row["cell_id"],row["arm"]):row for row in metrics}; summary={}
 for candidate,reference in (("D1_BASE","B3_FROZEN"),("J2_BLOCK_PLUS_CROSS_ADAM_R2","D1_BASE"),("J2_BLOCK_PLUS_CROSS_ADAM_R2","B3_FROZEN")):
  key=candidate+"_vs_"+reference; family=defaultdict(list); cell_values=[]; ap_values=[]
  for cell,bundle in bundles.items():
