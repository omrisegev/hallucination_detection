...
def main():
    ...
    for kind in KINDS:
        ...
        for outer in range(5):
            ...
            assert m['valid'],'Report native failures separately before roster interpretation'
            ...
            row=dict(kind=kind,outer=outer,selected_canonical=len(m['active']),
            ...
            rows.append(row)
        average=np.mean(np.abs(np.asarray(weights)),axis=0);mass={}
        for i,value in enumerate(average):mass[family(i)]=mass.get(family(i),0.)+float(value)
        np.testing.assert_allclose(sum(mass.values()),1.,atol=1e-12)
        summary[kind]=dict(canonical_counts=kept,original_retained_folds=dict(zip(ORIGINAL,map(int,counts))),
    ...
    result=dict(status='COMPLETE',scope='descriptive trained membership, not quality attribution',
        note='Exact aliases count once canonically; expanded weights split their mass. Active membership can include zero readout weights.',
    ...
    for name in ORIGINAL:
        lines.append(f"| {name} | {summary['base']['original_retained_folds'][name]}/5 | {summary['near_copies']['original_retained_folds'][name]}/5 |")
