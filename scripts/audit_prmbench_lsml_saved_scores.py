...
OUT = ROOT / "results/prmbench_lsml_month_audit_v4"
...
def main():
    ...
    evidence_dir = ROOT / ".worktrees/readout-quickest-detection-v1/results/step_evidence_v1"
    evidence_path = evidence_dir / "OOF_STEP_SCORES.npz"
    evidence_answers = evidence_dir / "OOF_ANSWERS.csv"
    with evidence_answers.open(encoding="utf-8-sig",newline="") as f:
        evidence_rows = list(csv.DictReader(f))
    assert [r["uid"] for r in records] == [r["uid"] for r in evidence_rows]
    with np.load(evidence_path) as z:
        np.testing.assert_array_equal(z["offsets"],off)
        np.testing.assert_array_equal(z["labels"],labels)
        for k in ("evidence__all__plain__continuous_lsml","evidence__all__plain__equal_std"):
            scores[k] = z[k].astype(float)
    ladder_oof = ROOT / "scratch/prmbench_month_drive_inputs/OOF.npz"
    ladder_eval = ROOT / "scratch/prmbench_month_drive_inputs/EVALUATION.npz"
    ladder_run = ROOT / ".worktrees/consolidation-fusion-2026-09-22/results/digitfree20_ladder_v1/RUN.json"
    assert sha(ladder_oof)=="a1e449e974b2975d0dd05c1c42797b6f9d73c34b748ab181d40d588adc436e32"
    assert sha(ladder_eval)=="9c4589ccf4150403ab10dd61d77e93c07ef5e1330e83df1449eadfe17a2549cb"
    assert sha(jp)=="22fcbfd346db6565665b5358ac74393a4188e9047565ca2af44908a9e8d3f49d"
    with np.load(ladder_eval) as z:
        for k,v in (("offsets",off),("labels",labels),("folds",folds),
                    ("cells",[r["cell"] for r in records]),("groups",[r["group_id"] for r in records])):
            np.testing.assert_array_equal(z[k],v)
    historical = json.loads(ladder_run.read_text(encoding="utf8"))["metrics"]
    with np.load(ladder_oof) as z:
        assert len(z.files)==64
        for k in z.files: scores["ladder__"+k]=z[k].astype(float)
    ...
    for name, score in scores.items():
        ...
        if name.startswith("ladder__"):
            assert abs(np.nanmean(au)-historical[name.removeprefix("ladder__")]["within"]) < 1e-10
    ...
    pairs = [("bank11_continuous","bank11_equal"),("bank11_pc4_continuous","bank11_pc4_equal"),
             ...
             ("evidence__all__plain__continuous_lsml","evidence__all__plain__equal_std")]
    pairs += [("ladder__"+a,"ladder__"+b) for a,b in (
        ("affinity_K3_csml","affinity_K3_ceq"),("eq15_K4_jrel","eq15_K4_ceq"),
        ("eq15_K4_jrel","eq15_K4_csml"),("continuous_auto","equal20"),
        ("eq15_K4_jrel","equal20"))]
    comparisons = []
    B = 100000; rng = np.random.default_rng(20260923)
    # Local retrospective family only; does not correct all month-long selection.
    family = len(pairs)*3
    for a,b in pairs:
        ...
        for endpoint,d in draws.items():
            ...
            comparisons.append({"a":a,"b":b,"endpoint":endpoint,"delta":float(point),
                                ...
                                "ci_bonferroni":np.quantile(d,[.025/family,1-.025/family]).tolist(),
                                "family_K":family,
    ...
    sources = paths+[jp,npz,folds_path,meta_path,module_path,fit_path,helper_path,bank_path,evidence_path,evidence_answers,ladder_oof,ladder_eval,ladder_run,Path(__file__)]
    report={"schema":"saved-score-prmbench-audit-v4","no_new_fit":True,"no_new_inference":True,
            "drive_source_prefix":"gdrive:hallucination_detection/consolidated_results/local_backup_2026-09-17/atlas/",
            "ladder_methods_replayed":64,
            ...
            "limitations":["Retrospective calibration of OOF scores; nested fusion training not reconstructed.",
                            ...
                            "Ladder K4 was selected retrospectively; local contrast correction does not undo historical selection."],
    ...
    print(json.dumps({"row_count":len(rows),"contrasts":len(comparisons),"seconds":report["seconds"]}),flush=True)
