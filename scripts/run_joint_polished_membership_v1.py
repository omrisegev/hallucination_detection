...
from spectral_utils.joint_polished_membership import fit_polished_joint,score_noise_aware_joint
...
OUT=ROOT/'results/joint_polished_membership_v1'
...
MASS=ROOT/'results/joint_mass_membership_v1'
KINDS=('base','duplicates','noise','near_copies','structured_noise')
PAIRS=[('near_copies__polished','near_copies__feasible_reference'),('base__polished','base__mass_reference')]+[
    (k+'__polished','base__polished') for k in KINDS[1:]]
...
def references(data):
    ...
    with np.load(MASS/'SCORES.npz') as z:
        np.testing.assert_array_equal(z['gate'],data['gate'])
        for k in KINDS:scores[k+'__mass_reference']=z[k+'__mass']
    return scores
...
def main():
    ...
    modules=('joint_mass_polish','joint_polished_groups','joint_polished_structure','joint_polished_membership','joint_polished_refinement','joint_staged_membership','joint_sparse_membership','joint_sparse_refinement',
    ...
    files=[Path(__file__),ROOT/'docs/experiments/JOINT_POLISHED_REFINEMENT_V1.md',ROOT/'tests/test_joint_polished_groups.py',ROOT/'tests/test_joint_mass_polish.py',ROOT/'tests/test_joint_polished_integration.py',MASS/'MASS_SOLVER_FAILURE_INPUTS.npz',ROOT/'tests/test_joint_polished_refinement.py',
        *[ROOT/f'spectral_utils/{m}.py' for m in modules],
        *[p/name for p in (OLD,HARD,PREVIOUS,ROBUST,LAST,MINIMAX,MASS) for name in ('SCORES.npz','MANIFEST.json','AUDIT.json')]]
    manifest=dict(schema='joint-polished-membership-v1',hashes={str(p):sha(p) for p in files})
    ...
    for kind in KINDS:
        x=bank(data,base,uids,kind);scores[kind+'__polished']=np.full(len(x),np.nan)
        ...
        for outer in range(5):
            ...
            if path.exists():meta=json.loads(path.read_text());model=meta['model']
                ...
                model=fit_polished_joint(*args,anchor_index=ANCHOR,seed=399170+outer,notify=notify)
                ...
                dump(path,meta)
            scores[kind+'__polished'][~rows]=score_noise_aware_joint(model,x[~rows],allow_anchor_fallback=True)
        ...
        if kind=='base':
            base_error=float(np.max(np.abs(scores['base__polished']-scores['base__old'])))
    ...
    preservation={k:bool(native[k]==13769 and ci[k+'__polished minus base__polished']['pb']['low']>-.01 and
        ci[k+'__polished minus base__polished']['within']['low']>-.002) for k in KINDS[1:]}
    result=dict(status='COMPLETE',metrics={k:scalar_metrics(m) for k,m in metrics.items()},
        ...
        fresh_fits=25,equivalent_replays=0,baseline_replay_max_error=base_error,baseline_preserved=bool(native['base']==13769 and ci['base__polished minus base__previous']['pb']['low']>-.01 and ci['base__polished minus base__previous']['within']['low']>-.002))
