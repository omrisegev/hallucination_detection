#!/usr/bin/env python
"""
eval_a6_pruned_direct.py — Fast direct evaluation of fixed a6.pruned_dufs across all 25 in-scope cells.
Uses 1-seed fast gate training to evaluate in ~5 seconds total.
"""
...
def main():
    print("Running direct fast benchmark of fixed a6.pruned_dufs...", flush=True)
    rows = []
    ...
    for domain, cell_key, fd, labels in iter_cells(data_dir=DATA_DIR, derived_views_pkl=DERIVED_PKL, trace_cells_pkl=TRACE_PKL):
        ...
        for vres in variants:
            if vres['variant'] == 'a6.pruned_dufs':
                ...
                print(f"Cell {cell_key}: n_selected={len(vres['cols'])}, AUROC={er['auroc']:.4f}, fallback={is_fallback}", flush=True)
