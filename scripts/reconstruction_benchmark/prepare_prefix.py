#!/usr/bin/env python3
"""Prepare one independent causal-prefix reconstruction build."""
...
from spectral_utils.reconstruction_benchmark.prefix_preparation import prepare_prefix_build  # noqa: E402
...
def main() -> None:
    ...
    if status.strip() and not args.allow_dirty_debug:
        raise RuntimeError("scientific prefix preparation requires a clean worktree")
    manifest = prepare_prefix_build(
        repo=REPO,
    ...
    print(json.dumps({
        "release_id": args.release_id,
        "build": args.build,
        "observations": manifest["expected_scores"]["observations"],
        "evaluation_traces": manifest["private_labels"]["rows"],
        "labels_exposed_to_fit": manifest["labels_exposed_to_fit"],
