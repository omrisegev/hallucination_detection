#!/usr/bin/env python3
"""Certify frozen localization evaluation A/B builds with split provenance."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--populations", type=Path, default=DEFAULT_POPULATION_REGISTRY,
    )
    parser.add_argument("--source-root", type=Path, default=DEFAULT_SOURCE_ROOT)
    parser.add_argument(
        "--score-verifier-repo", type=Path, required=True,
        help="Separate clean checkout at the score-frozen d96 commit.",
    )
    parser.add_argument(
        "--evaluation-producer-repo", type=Path, required=True,
        help=(
            "Separate clean checkout exactly matching the producer snapshot "
            "recorded in both evaluation manifests."
        ),
    )
    parser.add_argument(
    ...
    certificate = verify_localization_evaluation_ab_release(
        ...
        evaluation_producer_repo=args.evaluation_producer_repo,
