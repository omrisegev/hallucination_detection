"""One fixed six-column RBM weight-shrinkage test, using the frozen evaluator."""
import argparse
...
from spectral_utils.rbm_weight_shrinkage import METHODS, fit_all, LAMBDA
...
NAMES={'rbm':'Six-moment RBM', 'rbm_initial':'Six-moment RBM before learning', 'rbm_shrinkage':'Six-moment RBM with weight shrinkage'}
...
def manifest_for(source,v2,temporal,varentropy):
    ...
    manifest.update(schema='rbm-weight-shrinkage-v1',methods=list(METHODS),regularization=LAMBDA)
    files=[Path(__file__),ROOT/'spectral_utils/moment_rbm_fusion.py',ROOT/'spectral_utils/rbm_m3_powers.py', ROOT/'spectral_utils/rbm_weight_shrinkage.py',ROOT/'spectral_utils/surprisal_power_fusion.py',ROOT/'spectral_utils/varentropy_contribution_fusion.py', ROOT/'spectral_utils/deem_adapter.py',ROOT/'spectral_utils/deem_b3_contract_ablation.py',ROOT/'spectral_utils/residual_graph_deem.py',
           ROOT/'scripts/run_varentropy_contribution_fusion.py',
           ROOT/'docs/experiments/RBM_WEIGHT_SHRINKAGE_V1.md',
    ...
    files += [dependency_dir(source)/'RESULT_REVIEW.json',ROOT/'scripts/test_rbm_weight_shrinkage.py']
...
def evaluate(con,records,joined,temporal,v2,varentropy):
    ...
    primary={('rbm_shrinkage','rbm')}
    pairs=[('rbm_shrinkage','rbm'), ('rbm_shrinkage','rbm_initial')]
    pairs += [('rbm_shrinkage','ref__'+ref) for ref in ('entropy','k15__raw','k15__equal','k50__raw')]
    ...
    for m,t in telemetry.items():
        ...
        weights[m]=dict(note='Standardized weights; raw and oriented distances distinguish latent-state sign changes.',
            mean_oriented_weights=np.mean(np.asarray(saved)[:,:6],axis=0).tolist() if saved else [],
            raw_distance_quantiles=np.quantile([d['raw_weight_distance_from_initial'] for d in diagnostics],[0,.25,.5,.75,1]).tolist() if diagnostics else [],
            oriented_distance_quantiles=np.quantile([d['oriented_weight_distance_from_initial'] for d in diagnostics],[0,.25,.5,.75,1]).tolist() if diagnostics else [])
    payload=dict(schema='rbm-weight-shrinkage-v1',n_answers=len(records),n_steps=int(joined['offsets'][-1]),
