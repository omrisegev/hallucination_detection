...
Worktrees: the main checkout and `.worktrees/a6-s0b` (which holds master) are kept; the other 27 are
...
## B5. Cleanup completed (2026-09-17, final)

Drive backup verified: 243 objects, 1.25 GiB, at
`gdrive:hallucination_detection/consolidated_results/local_backup_2026-09-17/`, including every input the
frozen candidate depends on (its development scores and manifest, the bank extraction, the BOCPD input
and historical scores, the token-level telemetry features, metadata and step spans).

All 27 other worktrees were then removed; the main checkout and `.worktrees/a6-s0b` (master) remain, and
all 47 branches still exist. About 35 GB of disk was freed. Note the main checkout still carries
uncommitted edits to CLAUDE.md, HISTORY.md, PROGRESS.md and Research_Directions.md from the Codex line;
