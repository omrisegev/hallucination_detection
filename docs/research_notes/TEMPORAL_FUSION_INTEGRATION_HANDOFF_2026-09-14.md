...
recomputed from their eight cells. Large cluster output archives are not included. The 22 frozen runner inputs
are now included; new binary inputs use Git LFS (see INPUTS_REVIEW.json). These results are full development evidence, not untouched
...
After git lfs pull, the new worktree itself supplies all 22 registered inputs
and can be used as --source-root. See the second-machine instructions.
