...
the 30 registered gray-box telemetry features and emits
...
30 continuous telemetry measurements directly into a new visible energy model is
...
- the 30 gray-box telemetry features already extracted from one generation;
- their frozen semantic provenance families;
- unlabeled feature marginals within the current cell;
- response length as an explicitly declared nuisance and as one of the original
  30 Global features;
...
- Bundle: `results/dependency_fusion_raw/cells.npz`
...
## The 30-feature input contract

The primary DEEM input contains all 30 registered features in the exact order
recorded in the run definition. The six provenance groups are frozen by
...
removed from the primary 30-feature test. A separately named `stable26`
sensitivity removes them exactly as the existing contract specifies. Full-30
and stable-26 are both predeclared; neither may be selected after outcomes.

Before any fit, an inventory stage must verify all 30 names, finite values, row
identity, label alignment, and feature signs in every cell. Missing-feature
imputation is forbidden. If a cell lacks the exact 30-feature schema, the
scientific run stops and the source must be repaired or the protocol amended
...
learner predictions accepted by standard DEEM.
...
The energy contrast is constrained to reconstruct from six provenance blocks:

\[
\ell_\theta(x)=b+\sum_{g=1}^{6}c_{\theta,g}(x_g).
\]

This makes each family contribution explicit while preserving all 30 atomic
features inside its family network. If a low-rank interaction extension is
tested, it must appear as a named seventh block `interaction`, and

\[
\ell=b+\sum_g c_g+c_{\mathrm{interaction}}
\]

must still reconstruct numerically. The additive model is primary; the
interaction model is a registered sensitivity and cannot rescue it post hoc.
...
1. Split rows deterministically into five folds, stratifying only by target-blind
   length quantiles and stable row ID. Natural labels are never read.
2. Fit Model 1 on four folds and emit held-fold `ell_0` and family contributions
   `c_ig` on the fifth fold.
3. Repeat until every row has out-of-fold baseline values.
4. On donor folds only, fit a frozen nuisance regression for each contribution:
...
\[
d_R(i,j)^2=\sum_g\frac{1}{6}\frac{1}{|g|}
\lVert r_{ig}-r_{jg}\rVert_2^2.
\]

Family balancing prevents the 14 entropy-dynamics coordinates from dominating
the one-dimensional entropy-level and structural groups merely by count.
...
It does not delete features from the DEEM energy model. All 30 original features
continue to enter Model 1.

To reduce self-confirming geometry, gates are learned cross-view: the gate for
family `g` is estimated against a graph built from the other five families, not
from `g` itself. Fits are cross-fitted across rows and aggregated with equal
family mass. Gate normalization, sparsity penalty, optimizer, and seeds are fixed
before outcomes. A gate is not permitted to depend on correctness, hallucination,
or per-feature AUROC.
...
### Optional feature-family Laplacian

A six-node family graph may be estimated from cross-fitted residual dependencies
...
This is a secondary arm. An atomic 30-feature residual-covariance Laplacian is not
...
|---|---|---|---|
| B0 | `iu_pcr_full30` | direct incumbent under matched input contract | comparator |
| B1 | `deem30_hard_reproduction` | paper-aligned hard conversion | diagnostic baseline |
| B2 | `deem30_soft_rank_repaired` | repaired soft-rank adapter | baseline candidate |
| B3 | `deem30_continuous_additive` | proposed continuous-visible DEEM, no graph | core primary |
| G0 | `deem30_raw_graph_uniform_target` | raw-feature graph, target-smooth | circularity/nuisance control |
| G1 | `deem30_raw_graph_dufs_target` | raw DUFS graph, target-smooth | residualization ablation |
| G2 | `deem30_residual_graph_uniform_target` | residual graph, equal metric | graph primary A |
| G3 | `deem30_residual_graph_dufs_target` | residual graph, DUFS metric | graph primary A |
| G4 | `deem30_residual_graph_dufs_nuisance` | residual graph, explicit nuisance latent | graph primary B |
| G5 | `deem30_family_feature_laplacian` | six-family feature Laplacian | secondary |

Every arm is also run at `lambda=0`. For G2--G5, `lambda=0` must reproduce B3
within the registered numerical tolerance. The stable-26 schema is a named
sensitivity, not an extra candidate-search dimension.
...
2. `node_permuted_graph`: graph nodes are permuted while preserving graph degree
   and spectrum.
3. `random_gate_graph`: nonnegative random gates with matched effective feature
   count and family mass.
...
At minimum, generate deterministic fixtures for:
...
- exact 30-feature name/order/sign contract on all 24 cells;
...
At least 90% of the 24 cells must be graph-eligible for a graph-level claim.
...
overwrites the immutable score-freeze manifest.
...
Every rerun refits all outcome-dependent evaluation summaries and selects the
maximum statistic over graph hypotheses, supports, and eligible headline arms.
Use `B=199` for development and `B=999` before emitting a candidate for new-data
validation. Exact and CRT promotion are co-required. Tie seeds remain robustness
dimensions and are not multiplied into the sample count.
...
The following names are reserved for the later implementation; this document
...
1. **Architecture diagram:** 30 risk-oriented features -> six contribution
   families -> baseline DEEM -> cross-fitted residuals -> DUFS metric -> frozen
...
- the exact 30-feature inventory cannot be reconstructed on all 24 cells;
