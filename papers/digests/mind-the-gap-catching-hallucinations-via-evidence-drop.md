...
There are **no supervised PRMs, no critic/judge LLMs, no external verifiers** anywhere in the
comparison set — PRMs (Lightman et al., Math-Shepherd) are discussed only in Related Work and
dismissed as requiring step-level annotations; they are never run.
...
better**; Step-level Localization Accuracy (SLA). **The paper never reports ProcessBench F1 and
never reports clean-trace abstention accuracy** — SLA is measured on erroneous traces only.
...
- **Its comparison set is narrow (intrinsic-uncertainty heuristics only, no PRMs/critics/verifiers)
  and it never reports ProcessBench F1 or clean-trace abstention** — our GL-LIU benchmark expansion
  (uPRM audit, transparent token baselines, PRM/critic ceilings in separate access categories) is a
  strictly stronger comparison table than the field's current practice for this task.
