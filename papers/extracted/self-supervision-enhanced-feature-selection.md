...
1
INTRODUCTION
High-dimensional datasets are increasingly prevalent in a variety of ﬁelds, including critical do-
...
3
PROBLEM FORMULATION
Let X = (X1, · · · , Xp) ∈X p and Y ∈Y be random variables for the high-dimensional input
...
subject to |S| ≤δ
(1)
where δ constrains the number of selected features and ℓY (y, y′) = −PC
c=1 yc log y′
c for C-way
...
Let M = (M1, · · · , Mp) ∈{0, 1}p be binary random variables governed by distribution pM, whose
...
˜x ≜m ⊙x + (1 −m) ⊙¯x
(2)
where ¯x = Ex∼pX[x] and ⊙indicates element-wise multiplication. Here, we replace not-selected
...
i
(3)
where f is implemented as a neural network and β is a balancing coefﬁcient that controls the number
...
Our approach is deﬁned by the selection probability π, that governs the gate vector generation
...
Deﬁne π = (π1, · · · , πp) ∈[0, 1]p to be the selection probability that governs the multivariate
...
To solve these pretext tasks, we formally deﬁne the encoder fθ and two auxiliary network compo-
nents hψ1, hψ2 that are temporarily employed during the Self-Supervision Phase:
• Encoder, fθ : X p →Z, that takes as input either the selected subset of features ˜x or the original
...
i
(5)
where α is a coefﬁcient chosen to balance between the two pretext task losses: ℓX(x, ˆx) = ∥x−ˆx∥2
2
and ℓM(m, ˆm) = −Pp
...
Formally, given selection probability π = (π1, · · · , πp) and the multivariate uniform random
...

(6)
where σ(x) = (1 + exp(−x))−1 is the sigmoid function. Such a relaxation is parameterized by π,
...
SEFS
0.91±0.03
discovered features on the target tasks: we ﬁrst conduct feature selection based on each method and
...
To motivate and conﬁrm our intuition of applying self-supervision and correlated gate vectors in
...
Qualitative Results. Two interesting features identiﬁed by SEFS but that were not discovered by
...
2019), 2019.
Sercan ¨Omer Arik and Tomas Pﬁster. Tabnet: Attentive interpretable tabular learning. arXiv preprint
...
David A Belsley, Edwin Kuh, and Roy E Welsch. Regression diagnostics: Identifying inﬂuential
...
Ron Kohavi and George H. John. Wrappers for feature subset selection. Artiﬁcial Intelligence, 97
(1):273–324, 1997. doi: https://doi.org/10.1016/S0004-3702(97)00043-X.
Ludmila I. Kuncheva, Clare E. Matthews, ´Alvar Arnaiz-Gonz´alez, and Juan J. Rodr´ıguez. Feature
...
Huan Liu and Rudy Setiono. A probabilistic approach to feature selection - a ﬁlter solution. In
...
Milan Meloun, Jiˇr´ı Militk`y, Martin Hill, and Richard G Brereton. Crucial problems in regression
modelling and their solutions. Analyst, 127(4):433–450, 2002.
Fan Min, Qinghua Hu, and William Zhu.
...
nu−1
repeat
Sample a mini-batch of nmb unlabeled data samples: {xi}nmb
i=1 ∼Du
for i = 1, · · · , nmb do
...
nu−1
repeat
Sample a mini-batch of nmb labeled data samples: {xi, yi}nmb
i=1 ∼Dl
for i = 1, · · · , nmb do
Sample gate vector: ˜mi ∼Relaxed-MultiBern(π; R)
Generate feature subset: ˜xi ←˜m ⊙xi + (1 −˜m) ⊙¯x
...
For the Semi-Supervision Phase, coefﬁcients (α, π) and the hyper-parameters – including the num-
...
For the Supervision Phase, coefﬁcient β and the hyper-parameters – including the number of nodes
...
fφ ◦fθ(x)).
The permitted values of coefﬁcients (α, β, π) and the hyper-parameters are listed in Table S.1.
B.2
BENCHMARKS
...
7The speciﬁcation of the machine is: CPU – Intel Core i7-8700K, GPU – NVIDIA GeForce GTX 1080Ti,
and RAM – 64GB DDR4
...
• L-Score10: The Laplacian Score (L-Score) (He et al., 2005) is an unsupervised (ﬁlter) feature
...
To quantify the amount of collinearity, we calculated the variance inﬂation factor (VIF), which is
...
Qualitative Results. In the main manuscript, we validated the features identiﬁed by SEFS for
...
1.405±0.03
1.389±0.03
of the top 10 ranked features selected by SEFS (Table S.6b), with ﬁve such features not proposed
...
In addition, we also ﬁnd evidence of the importance of JUN (Riera-Sans & Behrens, 2007),
...
Published as a conference paper at ICLR 2022
APPENDIX REFERENCES
Frederick R. Adler and Theodore G. Liou. The dynamics of disease progression in cystic ﬁbrosis.
PLOS ONE, 11(6):1–17, 06 2016.
Riyad Bendardaf, Fatemeh Saheb Sharif-Askari, Narjes Saheb Sharif-Askari, Kari Syrj¨anen, and
...
Przemysław Duda, Shaw M. Akula, Stephen L. Abrams, Linda S. Steelman, Alberto M. Martelli,
...
Mirco Gali`e. Ras as supporting actor in breast cancer. Frontiers in oncology, 9:1199–1199, Nov
...
Llu´ıs Riera-Sans and Axel Behrens. Regulation of αβ/γδ t cell development by the activator protein
